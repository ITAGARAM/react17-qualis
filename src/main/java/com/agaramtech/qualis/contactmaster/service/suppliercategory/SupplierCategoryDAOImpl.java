...
import com.agaramtech.qualis.subcontracttestdetail.pojo.SupplierType;
...
//				+ userInfo.getNmastersitecode() + " and s.nsuppliercatcode > 0";
		...
		final String strQuery = "select s.nsuppliercatcode,s.nsuppliertypecode, s.ssuppliercatname, s.sdescription, s.nsitecode, s.nstatus, "
				+ "COALESCE(st.jsondata->'ssuppliertypename'->>'" + userInfo.getSlanguagetypecode()
				+ "',st.jsondata->'ssuppliertypename'->>'en-US') as ssuppliertypename "
				+ " from suppliercategory s,suppliertype st where s.nstatus = "
				+ Enumeration.TransactionStatus.ACTIVE.gettransactionstatus()
				+ " and s.nsuppliertypecode = st.nsuppliertypecode and st.nsitecode = " + userInfo.getNmastersitecode()
				+ " and s.nsitecode = " + userInfo.getNmastersitecode() + " and st.nstatus = "
				+ Enumeration.TransactionStatus.ACTIVE.gettransactionstatus() + " and s.nsuppliercatcode > 0";
	...
	// ALPD-5337 Supplier Category -supplier type field added, to check add, edit,
	// delete, view operation.
	@Override
	public ResponseEntity<Object> getSupplierType(Map<String, Object> inputMap, final UserInfo userInfo) throws Exception {

		final String strQuery = "select nsuppliertypecode,coalesce(g.jsondata->'ssuppliertypename'->>'"
				+ userInfo.getSlanguagetypecode() + "',"
				+ " g.jsondata->'ssuppliertypename'->>'en-US') as ssuppliertypename,ndefaultstatus,nsitecode,nstatus "
				+ " from suppliertype g where g.nsuppliertypecode > 0 and g.nsitecode = " + userInfo.getNmastersitecode()
				+ " and g.nstatus= " + Enumeration.TransactionStatus.ACTIVE.gettransactionstatus();

		return new ResponseEntity<>((List<SupplierType>) jdbcTemplate.query(strQuery, new SupplierType()),
				HttpStatus.OK);
	}
...
//				+ userInfo.getNmastersitecode() + " and s.nsuppliercatcode = " + nsuppliercatcode;
		...
		final String strQuery = "select s.nsuppliercatcode,s.nsuppliertypecode, s.ssuppliercatname, s.sdescription, s.nsitecode, s.nstatus, "
				...
				+ " from suppliercategory s,suppliertype st where s.nstatus = "
				+ Enumeration.TransactionStatus.ACTIVE.gettransactionstatus()
				+ " and s.nsuppliertypecode = st.nsuppliertypecode and " + " st.nstatus = "
				+ Enumeration.TransactionStatus.ACTIVE.gettransactionstatus() + " and s.nsuppliercatcode = "
				+ nsuppliercatcode;
...
//					SeqNoContactMaster.class, "nsequenceno");
			...
			nsequenceno++;
			String insertquery = "Insert into suppliercategory (nsuppliercatcode,nsuppliertypecode, ssuppliercatname, sdescription, dmodifieddate, nsitecode, nstatus) "
					   + "values("+nsequenceno+","+contactmasterSupplierCategory.getNsuppliertypecode()+",N'"+stringUtilityFunction.replaceQuote(contactmasterSupplierCategory.getSsuppliercatname())+"',N'"+stringUtilityFunction.replaceQuote(contactmasterSupplierCategory.getSdescription())+"', '"
	...
	public ResponseEntity<Object> updateSupplierCategory(SupplierCategory contactmasterSupplierCategory,
		...
		if (supplierCategory == null) {
			...
			if (suppliercategoryList.isEmpty()) {
				final String updateQueryString = "update suppliercategory set nsuppliertypecode = "+contactmasterSupplierCategory.getNsuppliertypecode()+",ssuppliercatname=N'"
						+ stringUtilityFunction.replaceQuote(contactmasterSupplierCategory.getSsuppliercatname())
						+ "', sdescription =N'"
		...
		if (suppliercategory == null) {
			...
			if (validRecord) {
				...
				final String updateQueryString = "update suppliercategory set nstatus = "
						...
						+ contactmasterSupplierCategory.getNsuppliercatcode() + " and nsitecode ="
