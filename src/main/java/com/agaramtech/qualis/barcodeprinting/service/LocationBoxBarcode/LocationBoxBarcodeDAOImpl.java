...
import com.agaramtech.qualis.barcodeprinting.service.RepositoryIdBarcode.RepositoryIdBarcodeDAOImpl;
...
@AllArgsConstructor
@Repository
public class LocationBoxBarcodeDAOImpl implements LocationBoxBarcodeDAO{
	
	private static final Logger LOGGER = LoggerFactory.getLogger(LocationBoxBarcodeDAOImpl.class);
	...
	public ResponseEntity<Object> getLocationBoxBarcode(UserInfo userInfo,int nbarcodefiltertypecode) throws Exception {
		...
		final String strQuery = "select *,coalesce(jsondata->'sbarcodefiltertypename'->>'"
				...
				+ " jsondata->'sbarcodefiltertypename'->>'en-US') as sdisplaystatus from barcodefiltertype where nformcode="+Enumeration.QualisForms.LOCATIONBOXBARCODE.getqualisforms()+" "+conditionValue+" order by  1";
		...
		if (!lstBarcodeFilterType.isEmpty()) {
			...
			outputMap.put("LocationBoxBarcodeData", lstquery);
