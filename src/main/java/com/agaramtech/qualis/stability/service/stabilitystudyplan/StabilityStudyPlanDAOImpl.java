...
//		JSONObject actionType = new JSONObject();
		...
		final List<StbStudyPlan> uncancelledUnRejectedList = lstStatus.stream()
				.filter(x -> x.getNtransactionstatus() != Enumeration.TransactionStatus.CANCELED.gettransactionstatus()
						...
						&& x.getNtransactionstatus() != Enumeration.TransactionStatus.APPROVED.gettransactionstatus()
...
//				int subSampleStatus = mainSampleStatus;
			...
			/*int nSeqNo = (int) jdbcUtilityFunction.queryForObject(
				...
				nSeqNo++;
				Map<String, Object> dateValue = registrationSample.getJsonuidata();
			
			String sQuery = "INSERT INTO stbschedulertransaction (" + "nstbschedulertransactioncode, "
					+ "nstbregistrationcode, " + "nstbregistrationsamplecode, " + "npreregno, "
					+ "dscheduleoccurrencedate, " + "dtransactiondate, " + "noffsetdtransactiondate, "
					+ "ntransdatetimezonecode, " + "ntransactionstatus, " + "nactivestatus, " + "nsitecode, "
					+ "nstatus" + ") VALUES (" + nSeqNo + ", " + npreregno + ", "
					+ nregsamplecode + ", " + Enumeration.TransactionStatus.NA.gettransactionstatus()
					+ ", " + "'" + dateValue.get("Occurrence Date") + "', " + "'"
					+ dateUtilityFunction.getCurrentDateTime(userInfo) + "', "
					+ dateUtilityFunction.getCurrentDateTimeOffset(userInfo.getStimezoneid()) + ", "
					+ userInfo.getNtimezonecode() + ", " + Enumeration.TransactionStatus.DRAFT.gettransactionstatus()
					+ ", " + Enumeration.TransactionStatus.YES.gettransactionstatus() + ", "
					+ userInfo.getNtranssitecode() + ", " + Enumeration.TransactionStatus.ACTIVE.gettransactionstatus()
					+ ");";
			sQuery=sQuery+ " update seqnostability set nsequenceno = " + nSeqNo
