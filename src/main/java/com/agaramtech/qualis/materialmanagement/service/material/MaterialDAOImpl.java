...
//							+ " and d1.value->>'value'='" + objUnit.getNunitcode()  + "'";
			...
			if (validRecord) {
				...
				//start-ALPDJ21-132
				String sQuery = "SELECT jsondata->>'ssystemfilename' AS system_filename "
				           + "FROM materialmsdsattachment WHERE nmaterialcode = " + inputMap.get("nmaterialcode");
				List<String> lstfileNames = jdbcTemplate.queryForList(sQuery, String.class);
			...
			if (objTestMaster != 0) {
				...
				if (Enumeration.ReturnStatus.SUCCESS.getreturnstatus().equals(sReturnString)) {
					...
					if (objTF != null) {
						...
						// Added by Ganesh on 28/11/2025 - To resolve s3 file override
						// start-ALPDJ21-132
						if((int) jsonobject.get("nattachmenttypecode") == Enumeration.AttachmentType.FTP.gettype()) {
							final String oldFileName = (String) objTF.getJsondata().get("sfilename");
						    final String newFileName = (String) jsonobject.get("sfilename");
						    ...
						    if (!oldFileExtension.equals(newFileExtension)) {
						        ftpUtilityFunction.deleteFTPFile(Arrays.asList((String) objTF.getJsondata().get("ssystemfilename")), "", objUserInfo);
						    }
							ssystemfilename = (String) jsonobject.get("sfilename");
						}
						//end-ALPDJ21-132
		...
		if (lstMaterial.size() != 0) {
			if (jsonArray1 != null) {
				...
				if (objTF != null) {
					...
					// Added by Ganesh on 28/11/2025 - ALPDJ21-132 - Delete in s3 storage
					ftpUtilityFunction.deleteFTPFile(Arrays.asList((String) objTF.getJsondata().get("ssystemfilename")), "", objUserInfo);
