...
public class AttachmentDAOImpl implements AttachmentDAO {
	...
	public ResponseEntity<? extends Object> updateSampleAttachment(MultipartHttpServletRequest request, UserInfo objUserInfo)
		...
		//Added by Ganesh on 28/11/2025 - To Resolve S3 file update override
		// start - ALPDJ21-132
		if(objRegistrationAttachment.getNattachmenttypecode() == Enumeration.AttachmentType.FTP.gettype()) {
			final String oldFileName = (String) validateAtttachment.getJsondata().get("sfilename");
			final String newFileName = (String) objRegistrationAttachment.getJsondata().get("sfilename");
		    ...
		    if (!oldFileExtension.equals(newFileExtension)) {
		        ftpUtilityFunction.deleteFTPFile(Arrays.asList((String) validateAtttachment.getJsondata().get("ssystemfilename")), "", objUserInfo);
		    }
		}
		// end - ALPDJ21-132
	...
	public ResponseEntity<? extends Object> deleteSampleAttachment(RegistrationAttachment objSampleAttachment, String npreregno,
		...
		if (validateAtttachment != null) {
			if (objSampleAttachment.getNattachmenttypecode() == Enumeration.AttachmentType.FTP.gettype()) {
				// Updated by Ganesh on 28/11/2025 - Delete file in S3 Storage
				ftpUtilityFunction.deleteFTPFile(Arrays.asList((String) validateAtttachment.getJsondata().get("ssystemfilename")), "", userInfo);
			} else {
...
//				LinkMaster objLinkMaster = jdbcTemplate.queryForObject(sQuery, new LinkMaster());
	...
	public ResponseEntity<? extends Object> updateTestAttachment(MultipartHttpServletRequest request, UserInfo objUserInfo)
		...
		//Added by Ganesh on 28/11/2025 - To Resolve S3 file update override
		// start - ALPDJ21-132
		if(objRegistrationTestAttachment.getNattachmenttypecode() == Enumeration.AttachmentType.FTP.gettype()) {
			final String oldFileName = (String) validateAtttachment.getJsondata().get("sfilename");
			final String newFileName = (String) objRegistrationTestAttachment.getJsondata().get("sfilename");
			String oldFileExtension = (oldFileName != null && oldFileName.contains("."))
				   ? oldFileName.substring(oldFileName.lastIndexOf('.') + 1)
				   : "";
		    String newFileExtension = (newFileName != null && newFileName.contains("."))
