...
import com.agaramtech.qualis.biobank.model.BiobankECatalogueRequestExternal;
import com.agaramtech.qualis.biobank.model.BiobankECatalogueRequestExternalDetails;
...
@AllArgsConstructor
@Repository
public class BiobankECatalogueExternalDAOImpl implements BiobankECatalogueExternalDAO {
	private static final Logger LOGGER = LoggerFactory.getLogger(BiobankECatalogueExternalDAOImpl.class);
	...
	@Override
	public ResponseEntity<Object> getBiobankECatalogueExternal(Map<String, Object> inputMap, UserInfo userInfo)
			throws Exception {
		...
		lstTransactionstatus = getBiobankECatalogueExternalRequestFormStatuses(userInfo);
		...
		List<BiobankECatalogueRequestExternal> lstBiobankECatalogueExternalRequests = getListBiobankECatalogueExternalRequest(
				returnMap, userInfo);
		if (!lstBiobankECatalogueExternalRequests.isEmpty()) {
			returnMap.put("lstBiobankECatalogueExternalRequests", lstBiobankECatalogueExternalRequests);
			returnMap.put("selectedBiobankECatalogueExternalRequest", lstBiobankECatalogueExternalRequests.getFirst());
			returnMap.put("lstBiobankECatalogueExternalDetails", getBiobankECatalogueExternalDetailsFromRequest(
					lstBiobankECatalogueExternalRequests.getFirst().getNbiobankecatreqexternalcode(), userInfo));

		} else {
			returnMap.put("lstBiobankECatalogueExternalRequests", null);
			returnMap.put("selectedBiobankECatalogueExternalRequest", null);
			returnMap.put("lstBiobankECatalogueExternalDetails", null);
		}
	...
	@Override
	public ResponseEntity<Object> getBiobankECatalogueExternalByFilterSubmit(Map<String, Object> inputMap,
		...
		List<TransactionStatus> lstTransactionstatus = getBiobankECatalogueExternalRequestFormStatuses(userInfo);
		returnMap.put("FilterStatus", lstTransactionstatus);
		returnMap.put("RealFilterStatuslist", lstTransactionstatus);
	...
	public ResponseEntity<Map<String, Object>> getActiveBiobankECatalogueExternalRequestForm(
		...
		Map<String, Object> returnMap = new HashMap<String, Object>();
		final int nbiobankecatreqexternalcode = Integer.valueOf(inputMap.get("nbiobankecatreqexternalcode").toString());
		final String sampleQry = "select "
				+ "bc.nbiobankecatreqexternalcode, bc.sformnumber,bc.nreqformtypecode, bc.nreceiversitecode, rs.ssitename as sreceivingsitename, "
				...
				+ "p.sprojecttitle as sprojecttitles, bc.nbioprojectcode, bc.nthirdpartycode, tp.sthirdpartyname "
				+ "from biobankecataloguerequestexternal bc, thirdparty tp, site rs, transactionstatus ts, timezone tz1, timezone tz2, requestformtype rft, "
				+ "bioproject p, formwisestatuscolor fwsc, colormaster cm " + "where bc.nbiobankecatreqexternalcode = "
				+ nbiobankecatreqexternalcode + " and bc.nreqformtypecode = rft.nreqformtypecode and rft.nstatus = "
				...
				+ Enumeration.TransactionStatus.ACTIVE.gettransactionstatus()
				+ " and bc.nthirdpartycode = tp.nthirdpartycode " + "and tp.nstatus = "
				+ Enumeration.TransactionStatus.ACTIVE.gettransactionstatus() + " " + "and tp.nsitecode = "
				+ userInfo.getNmastersitecode() + " " + ";";

		final BiobankECatalogueRequestExternal objBiobankECatalogueExternalRequest = (BiobankECatalogueRequestExternal) jdbcUtilityTemplateFunction
				.queryForObject(sampleQry, BiobankECatalogueRequestExternal.class, jdbcTemplate);
		if (objBiobankECatalogueExternalRequest != null) {
			returnMap.put("selectedBiobankECatalogueExternalRequest", objBiobankECatalogueExternalRequest);
			returnMap.put("lstBiobankECatalogueExternalDetails",
					getBiobankECatalogueExternalDetailsFromRequest(nbiobankecatreqexternalcode, userInfo));
		} else {
			returnMap.put("selectedBiobankECatalogueExternalRequest", null);
			returnMap.put("lstBiobankECatalogueExternalDetails", null);
		}
	...
	private List<BiobankECatalogueRequestExternalDetails> getBiobankECatalogueExternalDetailsFromRequest(
			final int nbiobankecatreqexternalcode, final UserInfo userInfo) throws Exception {

		final String catalogueDetailQry = "select bcd.nbiobankecatreqexternaldetailcode, bcd.nbiobankecatreqexternalcode, COALESCE(bcd.sparentsamplecode,'-') as sparentsamplecode, bcd.nreqnoofsamples, "
				...
				+ "from biobankecataloguerequestexternaldetails bcd, biobankecataloguerequestexternal bc, bioproject bp, site s, product pr, productcategory pc, timezone tz1 "
				+ "where bcd.nbiobankecatreqexternalcode = " + nbiobankecatreqexternalcode + " " + "and bcd.nstatus = "
				+ Enumeration.TransactionStatus.ACTIVE.gettransactionstatus() + " " + "and bcd.nsitecode = "
				+ userInfo.getNtranssitecode() + " "
				+ "and bc.nbiobankecatreqexternalcode = bcd.nbiobankecatreqexternalcode and bc.nstatus = "
				+ Enumeration.TransactionStatus.ACTIVE.gettransactionstatus() + " and bc.nsitecode = "
				...
				+ Enumeration.TransactionStatus.ACTIVE.gettransactionstatus() + " "
				+ "order by bcd.nbiobankecatreqexternaldetailcode desc";

		return jdbcTemplate.query(catalogueDetailQry, new BiobankECatalogueRequestExternalDetails());
	}

	private List<BiobankECatalogueRequestExternal> getListBiobankECatalogueExternalRequest(Map<String, Object> inputMap,
			UserInfo userInfo) {
		...
		if (obj instanceof Number) {
			nfilterstatus = ((Number) obj).intValue();
		}
		...
		final String sampleQry = "select "
				+ "bc.nbiobankecatreqexternalcode, bc.sformnumber, bc.nreceiversitecode, rs.ssitename as sreceivingsitename, "
				...
				+ "COALESCE(TO_CHAR(bc.dtransactiondate,'" + userInfo.getSsitedate() + "'), '') as stransactiondate, "
				+ "p.sprojecttitle as sprojecttitles, bc.nbioprojectcode, bc.nthirdpartycode, tp.sthirdpartyname "
				+ "from biobankecataloguerequestexternal bc, thirdparty tp, thirdpartyusermapping tpu, userroleconfig urc, site rs, transactionstatus ts, timezone tz1, timezone tz2, requestformtype rft, "
				...
				+ "and bc.nthirdpartycode = tp.nthirdpartycode " + "and tp.nstatus = "
				+ Enumeration.TransactionStatus.ACTIVE.gettransactionstatus() + " " + "and tp.nsitecode = "
				+ userInfo.getNmastersitecode() + " and tp.nisngs = "+Enumeration.TransactionStatus.YES.gettransactionstatus()
				+ " and tpu.nthirdpartycode = tp.nthirdpartycode and tpu.nusercode = "+userInfo.getNusercode() 
				+" and tpu.nstatus = "+Enumeration.TransactionStatus.ACTIVE.gettransactionstatus()
				+" and tpu.nsitecode = "+userInfo.getNmastersitecode()
				+ " and urc.nuserrolecode = tpu.nuserrolecode and urc.nneedngsflow = "+Enumeration.TransactionStatus.YES.gettransactionstatus()
				+" and urc.nstatus = "+Enumeration.TransactionStatus.ACTIVE.gettransactionstatus()
				+" and urc.nsitecode = "+userInfo.getNmastersitecode()+" and urc.nuserrolecode = "+userInfo.getNuserrole()
				+ " " + filterQry + " and bc.dtransactiondate between '" + fromDate
				+ "' and '" + toDate + "' " + "order by bc.nbiobankecatreqexternalcode desc";

		return jdbcTemplate.query(sampleQry, new BiobankECatalogueRequestExternal());
	}

	public List<TransactionStatus> getBiobankECatalogueExternalRequestFormStatuses(final UserInfo userInfo)
			throws Exception {
	...
	public List<Site> getSiteComboFromProject(final int nbioprojectcode, final UserInfo userInfo) throws Exception {
		...
		String ssitecode = String.valueOf(userInfo.getNtranssitecode());
		...
		final String strQuery = "SELECT s.nsitecode, s.ssitename, s.ndefaultstatus " + "  FROM site s "
				...
				+ "   AND s.nsitecode > 0 " + "   AND s.nsitecode IN (" + ssitecode + ") " + " ORDER BY s.ssitename;";
	...
	@Override
	public ResponseEntity<Object> getComboDataForCatalogueForAdd(Map<String, Object> inputMap, UserInfo userInfo)
			throws Exception {
		Map<String, Object> returnMap = new HashMap<>();
		Integer nThirdPartyCode = getThirdPartyCodeForUser(userInfo);
		if (nThirdPartyCode == null) {
			return new ResponseEntity<>(commonFunction.getMultilingualMessage("IDS_USERNOTINNGSTHIRDPARTY",
	...
	@Override
	public ResponseEntity<Object> createBiobankECatalogueExternalRequest(Map<String, Object> inputMap,
			UserInfo userInfo) throws Exception {
		Integer nThirdPartyCode = getThirdPartyCodeForUser(userInfo);
		if (nThirdPartyCode == null) {
			return new ResponseEntity<>(commonFunction.getMultilingualMessage("IDS_USERNOTINNGSTHIRDPARTY",
			...
			String sQuery = " lock  table lockbiobankecataloguerequestexternal "
					+ Enumeration.ReturnStatus.TABLOCK.getreturnstatus();
			jdbcTemplate.execute(sQuery);

			final String sSeqQuery = "select nsequenceno,stablename from seqnobiobankmanagement where stablename in (N'biobankecataloguerequestexternal',"
					+ "N'biobankecataloguerequestexternalhistory')";
			...
			int seqNoBgsiECatReq = seqNoMap.get("biobankecataloguerequestexternal") + 1;
			int seqNoBgsiECatReqHis = seqNoMap.get("biobankecataloguerequestexternalhistory") + 1;

			final String strformat = projectDAOSupport.getSeqfnFormat("biobankecataloguerequestexternal",
					"seqnoformatgeneratorbiobank", 0, 0, userInfo);

			String strInsertBiobankECatalogueExternal = "INSERT INTO public.biobankecataloguerequestexternal("
					+ "	nbiobankecatreqexternalcode, sformnumber, nreqformtypecode, nthirdpartycode, nbioprojectcode, nreceiversitecode, drequesteddate, ntzrequesteddate,"
					...
					+ seqNoBgsiECatReq + ", '" + strformat + "', " + nselectedreqformtypecode + ", "
					+ (int) nThirdPartyCode + ", " + nselectedbioprojectcode + ", " + nselectedsitecode + ", '"
					...
					+ ");";
			String strInsertBiobankECatalogueExternalHistory = "INSERT INTO public.biobankecataloguerequestexternalhistory("
					+ "	nbiobankecatreqexternalhistorycode, nbiobankecatreqexternalcode, nusercode, nuserrolecode, ndeputyusercode,"
			...
			String strSeqNoUpdate = "update seqnobiobankmanagement set nsequenceno=" + seqNoBgsiECatReq + " where"
					+ " stablename='biobankecataloguerequestexternal' and nstatus="
					+ Enumeration.TransactionStatus.ACTIVE.gettransactionstatus() + ";";
			strSeqNoUpdate += "update seqnobiobankmanagement set nsequenceno=" + seqNoBgsiECatReqHis + " where"
					+ " stablename='biobankecataloguerequestexternalhistory' and nstatus="
					+ Enumeration.TransactionStatus.ACTIVE.gettransactionstatus() + ";";

			inputMap.put("nbiobankecatreqexternalcode", seqNoBgsiECatReq);

			String rtnQry = insertBiobankECatalogueExternalDetails(inputMap, userInfo);

			jdbcTemplate.execute(strInsertBiobankECatalogueExternal + strInsertBiobankECatalogueExternalHistory + rtnQry
					+ strSeqNoUpdate);
			...
			Map<String, Object> returnMap = new HashMap<String, Object>();
			returnMap.putAll(
					(Map<String, Object>) getActiveBiobankECatalogueExternalRequestForm(inputMap, userInfo).getBody());
			BiobankECatalogueRequestExternal objBiobankECatalogueExternalRequest = (BiobankECatalogueRequestExternal) returnMap
					.get("selectedBiobankECatalogueExternalRequest");
			listBeforeUpdate.add(objBiobankECatalogueExternalRequest);
			multilingualIDList.add("IDS_ADDBIOBANKEXTERNALECATALOGUEREQUEST");
			List<BiobankECatalogueRequestExternalDetails> lstBiobankECatalogueExternalDetails = (List<BiobankECatalogueRequestExternalDetails>) returnMap
					.get("lstBiobankECatalogueExternalDetails");
			listBeforeUpdate.addAll(lstBiobankECatalogueExternalDetails);
			lstBiobankECatalogueExternalDetails.stream()
					.forEach(x -> multilingualIDList.add("IDS_ADDBIOBANKEXTERNALECATALOGUEREQUESTSAMPLE"));
			auditUtilityFunction.fnInsertAuditAction(listBeforeUpdate, 1, null, multilingualIDList, userInfo);

			return getBiobankECatalogueExternalByFilterSubmit(inputMap, userInfo);
		}
	}

	public String insertBiobankECatalogueExternalDetails(Map<String, Object> inputMap, final UserInfo userInfo)
			throws Exception {
		...
		final String sSeqQuery = "select nsequenceno,stablename from seqnobiobankmanagement where stablename in (N'biobankecataloguerequestexternaldetails',"
				+ "N'bioecataloguerequestdetails')";
		List<SeqNoBioBankManagement> lstSeqBioBank = jdbcTemplate.query(sSeqQuery, new SeqNoBioBankManagement());
		Map<String, Integer> seqNoMap = lstSeqBioBank.stream()
				.collect(Collectors.toMap(SeqNoBioBankManagement::getStablename,
						seqNoTestGroupmanagement -> seqNoTestGroupmanagement.getNsequenceno()));
		int seqNoBgsiECatDetails = seqNoMap.get("biobankecataloguerequestexternaldetails");

		String strBiobankECatalogueExternalDetails = "INSERT INTO public.biobankecataloguerequestexternaldetails("
				+ "	nbiobankecatreqexternaldetailcode, nbiobankecatreqexternalcode, nbioprojectcode, nproductcode,"
		...
		for (Map<String, Object> reqProduct : requestProductList) {
			...
			String sreqminvolume = "'" + stringUtilityFunction.replaceQuote(reqProduct.get("sreqminvolume").toString())
					+ "'";
			strBiobankECatalogueExternalDetails += "(" + seqNoBgsiECatDetails + ", " + nbiobankecatreqexternalcode
		...
		strBiobankECatalogueExternalDetails = strBiobankECatalogueExternalDetails.substring(0,
				strBiobankECatalogueExternalDetails.length() - 1) + ";";

		String strSeqNoUpdate = "update seqnobiobankmanagement set nsequenceno=" + seqNoBgsiECatDetails + " where"
				+ " stablename='biobankecataloguerequestexternaldetails' and nstatus="
				+ Enumeration.TransactionStatus.ACTIVE.gettransactionstatus() + ";";

		return (strBiobankECatalogueExternalDetails + strSeqNoUpdate);
	}
	...
	public ResponseEntity<Object> sendBiobankECatalogueExternalRequest(Map<String, Object> inputMap, UserInfo userInfo)
		...
		if (nThirdPartyCode == null) {
			...
			BiobankECatalogueRequestExternal objBiobankECatalogueExternalRequest = (BiobankECatalogueRequestExternal) returnMap
					.get("selectedBiobankECatalogueExternalRequest");
			if (objBiobankECatalogueExternalRequest != null && objBiobankECatalogueExternalRequest.getNthirdpartycode() == nThirdPartyCode) {
			if (objBiobankECatalogueExternalRequest != null && objBiobankECatalogueExternalRequest
					.getNtransactionstatus() == Enumeration.TransactionStatus.DRAFT.gettransactionstatus()) {

				listBeforeUpdate.add(objBiobankECatalogueExternalRequest);

				final List<BiobankECatalogueRequestExternalDetails> lstBiobankECatalogueExternalDetails = (List<BiobankECatalogueRequestExternalDetails>) returnMap
						.get("lstBiobankECatalogueExternalDetails");
				if (lstBiobankECatalogueExternalDetails != null && !lstBiobankECatalogueExternalDetails.isEmpty()) {
					...
					final String sSeqQuery = "select nsequenceno,stablename from seqnobiobankmanagement where stablename in ("
							+ "N'biobankecataloguerequestexternalhistory',N'bioecataloguereqapproval',N'bioecataloguerequestdetails',N'bioecataloguereqapprovalhistory')";
					List<SeqNoBioBankManagement> lstSeqBioBank = jdbcTemplate.query(sSeqQuery,
							new SeqNoBioBankManagement());
					Map<String, Integer> seqNoMap = lstSeqBioBank.stream()
							.collect(Collectors.toMap(SeqNoBioBankManagement::getStablename,
									seqNoTestGroupmanagement -> seqNoTestGroupmanagement.getNsequenceno()));
					int seqNoBiobankECatalogueExternalHistory = seqNoMap.get("biobankecataloguerequestexternalhistory")
							+ 1;
					...
					final String updQry = "Update biobankecataloguerequestexternal set ntransactionstatus = "
							...
							+ " where nbiobankecatreqexternalcode = "
							+ objBiobankECatalogueExternalRequest.getNbiobankecatreqexternalcode() + " and nsitecode = "
							+ userInfo.getNtranssitecode() + " and nstatus = "
							+ Enumeration.TransactionStatus.ACTIVE.gettransactionstatus() + "; ";

					String strInsert = "INSERT INTO public.biobankecataloguerequestexternalhistory("
							+ "	nbiobankecatreqexternalhistorycode, nbiobankecatreqexternalcode, nusercode, nuserrolecode, ndeputyusercode,"
							+ " ndeputyuserrolecode, ntransactionstatus, scomments, dtransactiondate, ntztransactiondate,"
							+ " noffsettransactiondate, nsitecode, nstatus)" + "	VALUES ("
							+ seqNoBiobankECatalogueExternalHistory + ","
							+ objBiobankECatalogueExternalRequest.getNbiobankecatreqexternalcode() + ","
					...
					strInsert += "update seqnobiobankmanagement set nsequenceno="
							+ seqNoBiobankECatalogueExternalHistory + " where"
							+ " stablename='biobankecataloguerequestexternalhistory' and nstatus="
							+ Enumeration.TransactionStatus.ACTIVE.gettransactionstatus() + ";";
					...
					String strInsertBioEcatReqApp = "INSERT INTO public.bioecataloguereqapproval("
							...
							+ Enumeration.TransferType.BIOBANKEXTERNAL.getntransfertype() + ", "
							+ userInfo.getNtranssitecode() + ", "
							+ objBiobankECatalogueExternalRequest.getNthirdpartycode() + ", '"
							+ objBiobankECatalogueExternalRequest.getSformnumber() + "', "
							+ objBiobankECatalogueExternalRequest.getNreqformtypecode() + ", '"
							+ dateUtilityFunction.getCurrentDateTime(userInfo) + "', " + userInfo.getNtimezonecode()
							+ ", " + dateUtilityFunction.getCurrentDateTimeOffset(userInfo.getStimezoneid()) + ", "
							+ Enumeration.TransactionStatus.REQUESTED.gettransactionstatus() + ", null, '"
							+ dateUtilityFunction.getCurrentDateTime(userInfo) + "', " + userInfo.getNtimezonecode()
							+ ", " + dateUtilityFunction.getCurrentDateTimeOffset(userInfo.getStimezoneid()) + ", "
							+ objBiobankECatalogueExternalRequest.getNreceiversitecode() + ", "
							+ Enumeration.TransactionStatus.ACTIVE.gettransactionstatus() + ");";
					...
					String strInsertBioEcatReqAppHis = "INSERT INTO public.bioecataloguereqapprovalhistory("
							...
							+ "," + seqNoEcatReqApproval + "," + userInfo.getNusercode() + ", "
					...
					strInsertBioEcatReqAppHis += "update seqnobiobankmanagement set nsequenceno="
							+ seqNoEcatReqApprovalHis + " where"
					...
					for (BiobankECatalogueRequestExternalDetails objBiobankECatalogueExternalDetail : lstBiobankECatalogueExternalDetails) {
						seqNoEcatReqAppDetails++;
						int nproductcode = objBiobankECatalogueExternalDetail.getNproductcode();
						int nreqnoofsamples = objBiobankECatalogueExternalDetail.getNreqnoofsamples();
						String sparentsamplecode = objBiobankECatalogueExternalDetail.getSparentsamplecode() != null
								&& !objBiobankECatalogueExternalDetail.getSparentsamplecode().isEmpty()
								&& !objBiobankECatalogueExternalDetail.getSparentsamplecode().equalsIgnoreCase("-")
										? "'" + stringUtilityFunction.replaceQuote(
												objBiobankECatalogueExternalDetail.getSparentsamplecode()) + "'"
										: null;
						String sreqminvolume = "'" + stringUtilityFunction
								.replaceQuote(objBiobankECatalogueExternalDetail.getSreqminvolume()) + "'";
						strBioECatalogueRequestDetails += "(" + seqNoEcatReqAppDetails + ", " + seqNoEcatReqApproval
								+ ", " + objBiobankECatalogueExternalDetail.getNbioprojectcode() + ", " + nproductcode
								+ ", " + sparentsamplecode + ", " + sreqminvolume + ", " + nreqnoofsamples
								+ ", null, null," + " null, " + "'" + dateUtilityFunction.getCurrentDateTime(userInfo)
					...
					outputMap.putAll(
							(Map<String, Object>) getActiveBiobankECatalogueExternalRequestForm(inputMap, userInfo)
									.getBody());

					final List<String> multilingualIDList = new ArrayList<>();
					listAfterUpdate.add(outputMap.get("selectedBiobankECatalogueExternalRequest"));

					multilingualIDList.add("IDS_SENDBIOBANKECATALOGUEREQUEST");
				...
				return new ResponseEntity<>(commonFunction.getMultilingualMessage("IDS_USERDOESNOTBELONGTOSAMEINSTITUTION",
						userInfo.getSlanguagefilename()), HttpStatus.CONFLICT);				
			}
		}
	}
	...
	@Override
	public ResponseEntity<Object> cancelBiobankECatalogueExternalRequest(Map<String, Object> inputMap,
			UserInfo userInfo) throws Exception {
		Integer nThirdPartyCode = getThirdPartyCodeForUser(userInfo);
		if (nThirdPartyCode == null) {
			return new ResponseEntity<>(commonFunction.getMultilingualMessage("IDS_USERNOTINNGSTHIRDPARTY",
			...
			if (objBiobankECatalogueExternalRequest != null && (objBiobankECatalogueExternalRequest
					.getNtransactionstatus() == Enumeration.TransactionStatus.DRAFT.gettransactionstatus()
					|| objBiobankECatalogueExternalRequest
							.getNtransactionstatus() == Enumeration.TransactionStatus.REQUESTED
				...
				final String ecatReqAppStr = "select necatrequestreqapprovalcode " + "  from bioecataloguereqapproval "
						+ " where sformnumber = N'" + objBiobankECatalogueExternalRequest.getSformnumber() + "' "
						+ "   and nsitecode   = " + objBiobankECatalogueExternalRequest.getNreceiversitecode() + " "
				...
				int seqNoBiobankECatalogueExternalHistory = jdbcTemplate.queryForObject(
				...
				int seqNoEcatReqApprovalHis = jdbcTemplate
				...
				String strInsert = "INSERT INTO public.biobankecataloguerequestexternalhistory("
						...
						"update seqnobiobankmanagement set nsequenceno=" + seqNoBiobankECatalogueExternalHistory
				...
				if (necatrequestreqapprovalcode != null) {
					...
					String strInsertBioEcatReqAppHis = "INSERT INTO public.bioecataloguereqapprovalhistory("
							...
							+ " ndeputyuserrolecode, ntransactionstatus, dtransactiondate, ntztransactiondate,"
							+ " noffsetdtransactiondate, nsitecode, nstatus)" + " VALUES (" + seqNoEcatReqApprovalHis
							+ "," + necatrequestreqapprovalcode + "," + userInfo.getNusercode() + ", "
							+ userInfo.getNuserrole() + ", " + userInfo.getNdeputyusercode() + ", "
							+ userInfo.getNdeputyuserrole() + ", "
							+ Enumeration.TransactionStatus.CANCELED.gettransactionstatus() + ", " + "'"
							+ dateUtilityFunction.getCurrentDateTime(userInfo) + "', " + userInfo.getNtimezonecode()
				...
				outputMap.putAll((Map<String, Object>) getActiveBiobankECatalogueExternalRequestForm(inputMap, userInfo)
						.getBody());

				final List<String> multilingualIDList = new ArrayList<>();
				listAfterUpdate.add(outputMap.get("selectedBiobankECatalogueExternalRequest"));
				multilingualIDList.add("IDS_CANCELBIOBANKECATALOGUEREQUEST");
				...
				return new ResponseEntity<>(
						commonFunction.getMultilingualMessage("IDS_SELECTDRAFTORREQUESTEDSTATUSRECORD",
	...
	public BiobankECatalogueRequestExternal getActiveBiobankECatalogueExternalRequestForValidation(
			final int nbiobankecatreqexternalcode, UserInfo userInfo) throws Exception {
		final String sampleQry = "select "
				+ "bc.nbiobankecatreqexternalcode, bc.sformnumber, bc.nreqformtypecode, bc.nreceiversitecode, rs.ssitename as sreceivingsitename, "
				+ "bc.drequesteddate, bc.ntzrequesteddate, tz1.stimezoneid as stzrequesteddate, "
				+ "COALESCE(TO_CHAR(bc.drequesteddate,'" + userInfo.getSsitedate() + "'), '') as srequesteddate, "
				...
				+ "bc.dtransactiondate, bc.ntztransactiondate, tz2.stimezoneid as stztransactiondate, "
				+ "COALESCE(TO_CHAR(bc.dtransactiondate,'" + userInfo.getSsitedate() + "'), '') as stransactiondate, "
				+ "p.sprojecttitle as sprojecttitles, p.nbioprojectcode, bc.nthirdpartycode, tp.sthirdpartyname  "
				+ "from biobankecataloguerequestexternal bc, thirdparty tp, site rs, transactionstatus ts, timezone tz1, timezone tz2, "
				+ "bioproject p, formwisestatuscolor fwsc, colormaster cm " + "where bc.nbiobankecatreqexternalcode = "
				+ nbiobankecatreqexternalcode + " and bc.nreceiversitecode = rs.nsitecode and rs.nstatus = "
				+ Enumeration.TransactionStatus.ACTIVE.gettransactionstatus() + " "
				+ "and bc.ntransactionstatus = ts.ntranscode and ts.nstatus = "
				+ Enumeration.TransactionStatus.ACTIVE.gettransactionstatus() + " "
				+ "and bc.ntzrequesteddate = tz1.ntimezonecode and tz1.nstatus = "
				...
				+ " and fwsc.ncolorcode=cm.ncolorcode and cm.nstatus = "
				+ Enumeration.TransactionStatus.ACTIVE.gettransactionstatus() +" "
				+ "and bc.nthirdpartycode = tp.nthirdpartycode "
		...
		return objBiobankECatalogueExternalRequest;
	}

	@Override
	public ResponseEntity<Object> createBiobankECatalogueExternalRequestSample(Map<String, Object> inputMap,
			UserInfo userInfo) throws Exception {
		Integer nThirdPartyCode = getThirdPartyCodeForUser(userInfo);
		if (nThirdPartyCode == null) {
			return new ResponseEntity<>(commonFunction.getMultilingualMessage("IDS_USERNOTINNGSTHIRDPARTY",
			...
			final int nbiobankecatreqexternalcode = Integer
					.valueOf(inputMap.get("nbiobankecatreqexternalcode").toString());
			BiobankECatalogueRequestExternal objBiobankECatalogueExternal = getActiveBiobankECatalogueExternalRequestForValidation(
					nbiobankecatreqexternalcode, userInfo);
			if (objBiobankECatalogueExternal != null && objBiobankECatalogueExternal.getNthirdpartycode() == nThirdPartyCode) {
			if (objBiobankECatalogueExternal != null && objBiobankECatalogueExternal
					.getNtransactionstatus() == Enumeration.TransactionStatus.DRAFT.gettransactionstatus()) {
				...
				String sparentsamplecode = inputMap.containsKey("sparentsamplecode")
						...
						&& objBiobankECatalogueExternal
								.getNreqformtypecode() == Enumeration.RequestFormType.PARENTSAMPLEBASED
										.getRequestFormType()
												? "'" + stringUtilityFunction.replaceQuote(
														inputMap.get("sparentsamplecode").toString()) + "'"
												: null;
				final int nreqformtypecode = objBiobankECatalogueExternal.getNreqformtypecode();
				...
				if (nreqformtypecode == Enumeration.RequestFormType.BIOSAMPLEBASED.getRequestFormType()
					...
					boolean isProductAvailable = isProductAvailableInForm(nbiobankecatreqexternalcode, nreqformtypecode,
					...
					if (!isProductAvailable) {
						...
						final String sreqminvolumeTxt = "'" + sreqminvolumeRaw + "'";
						...
						String availableCountQry = "SELECT COUNT(st.nsamplestoragetransactioncode) "
								...
								+ " WHERE st.nprojecttypecode = " + objBiobankECatalogueExternal.getNbioprojectcode()
								+ " " + "  AND st.nproductcode = " + nproductcode + " " + "  AND st.nsitecode = "
								+ objBiobankECatalogueExternal.getNreceiversitecode() + " "
						...
						if (navailablecount >= nreqnoofsamples) {
							...
							int seqNoBgsiECatDetails = jdbcTemplate.queryForObject(
									"select nsequenceno from seqnobiobankmanagement where stablename in (N'biobankecataloguerequestexternaldetails') and nstatus="
											+ Enumeration.TransactionStatus.ACTIVE.gettransactionstatus(),
									Integer.class);
							...
							String strBiobankECatalogueExternalDetails = "INSERT INTO public.biobankecataloguerequestexternaldetails("
									...
									+ " sparentsamplecode, sreqminvolume, nreqnoofsamples, naccnoofsamples, saccminvolume, "
									+ " dtransactiondate, ntztransactiondate, noffsetdtransactiondate, nsitecode, nstatus)"
									+ "	VALUES (" + seqNoBgsiECatDetails + ", " + nbiobankecatreqexternalcode + ", "
									+ objBiobankECatalogueExternal.getNbioprojectcode() + ", " + nproductcode + ","
							...
							String strSeqNoUpdate = "update seqnobiobankmanagement set nsequenceno="
									+ seqNoBgsiECatDetails + " where"
									+ " stablename='biobankecataloguerequestexternaldetails' and nstatus="
									+ Enumeration.TransactionStatus.ACTIVE.gettransactionstatus() + ";";

							jdbcTemplate.execute(strBiobankECatalogueExternalDetails + strSeqNoUpdate);
							Map<String, Object> outputMap = new HashMap<String, Object>();
							outputMap.putAll(
									(Map<String, Object>) getActiveBiobankECatalogueExternalRequestForm(inputMap,
											userInfo).getBody());
							outputMap.putAll((Map<String, Object>) getProductComboDataForSampleAdd(inputMap, userInfo)
									.getBody());

							final List<Object> listBeforeUpdate = new ArrayList<>();
							final List<String> multilingualIDList = new ArrayList<>();
							inputMap.put("nbiobankecatreqexternaldetailcode", seqNoBgsiECatDetails);
							BiobankECatalogueRequestExternalDetails objBiobankECatalogueExternalDetails = (BiobankECatalogueRequestExternalDetails) getActiveSampleDetail(
									inputMap, userInfo).getBody().get("selectedBiobankECatalogueExternalDetails");

							listBeforeUpdate.add(objBiobankECatalogueExternalDetails);
							multilingualIDList.add("IDS_ADDBIOBANKECATALOGUEREQUESTSAMPLE");
							...
							return new ResponseEntity<>(commonFunction.getMultilingualMessage(
									"IDS_REQUESTEDQUANTITYMORETHANAVAILABLEQUANTITY", userInfo.getSlanguagefilename()),
									HttpStatus.CONFLICT);
						}
					} else {
	...
	@Override
	public ResponseEntity<Object> updateBiobankECatalogueExternalRequestSample(Map<String, Object> inputMap,
			UserInfo userInfo) throws Exception {
		Integer nThirdPartyCode = getThirdPartyCodeForUser(userInfo);
		if (nThirdPartyCode == null) {
			return new ResponseEntity<>(commonFunction.getMultilingualMessage("IDS_USERNOTINNGSTHIRDPARTY",
			...
			if (objBiobankECatalogueExternal != null && objBiobankECatalogueExternal
				...
				if (objBiobankECatalogueExternalDetails != null) {
					...
					if (nreqformtypecode == Enumeration.RequestFormType.BIOSAMPLEBASED.getRequestFormType()
						...
						final String castExprB = "REPLACE(TRIM(" + sreqminvolumewithquotes + " COLLATE \"default\"), '"
								+ opForReplace + "', '.')::numeric";
						String parentSampleConditionStr = sparentsamplecode != null && objBiobankECatalogueExternal
								.getNreqformtypecode() == Enumeration.RequestFormType.PARENTSAMPLEBASED
										.getRequestFormType() ? " and st.sparentsamplecode = " + sparentsamplecode : "";
						...
						String availableCountQry = "SELECT COUNT(st.nsamplestoragetransactioncode) "
								...
								+ " WHERE st.nprojecttypecode = " + objBiobankECatalogueExternal.getNbioprojectcode()
								+ " " + "  AND st.nproductcode = "
								+ objBiobankECatalogueExternalDetails.getNproductcode() + " " + "  AND st.nsitecode = "
								+ objBiobankECatalogueExternal.getNreceiversitecode() + " " + "  AND (CASE "
						...
						if (navailablecount >= nreqnoofsamples) {
							...
							final String updQry = "Update biobankecataloguerequestexternaldetails set sreqminvolume = "
									+ sreqminvolumewithquotes + ", nreqnoofsamples = " + nreqnoofsamples
									+ ", sparentsamplecode = " + sparentsamplecode + ", dtransactiondate = '"
									+ dateUtilityFunction.getCurrentDateTime(userInfo) + "', noffsetdtransactiondate = "
									+ dateUtilityFunction.getCurrentDateTimeOffset(userInfo.getStimezoneid())
									+ ", ntztransactiondate = " + userInfo.getNtimezonecode()
									+ " where nbiobankecatreqexternaldetailcode = "
									+ objBiobankECatalogueExternalDetails.getNbiobankecatreqexternaldetailcode()
							...
							final List<String> multilingualIDList = new ArrayList<>();
							listAfterUpdate.add(outputMap.get("selectedBiobankECatalogueExternalDetails"));

							multilingualIDList.add("IDS_EDITBIOBANKECATALOGUEREQUESTSAMPLE");
	...
	@Override
	public ResponseEntity<Object> deleteBiobankECatalogueExternalRequestSample(Map<String, Object> inputMap,
			UserInfo userInfo) throws Exception {
		Integer nThirdPartyCode = getThirdPartyCodeForUser(userInfo);
		if (nThirdPartyCode == null) {
			return new ResponseEntity<>(commonFunction.getMultilingualMessage("IDS_USERNOTINNGSTHIRDPARTY",
			...
			if (objBiobankECatalogueExternal != null && objBiobankECatalogueExternal
				...
				if (objBiobankECatalogueExternalDetails != null) {
					...
					final String updQry = "Update biobankecataloguerequestexternaldetails set nstatus = "
					...
					final List<Object> listAfterUpdate = new ArrayList<>();
					listAfterUpdate.add(objBiobankECatalogueExternalDetails);
					final List<String> multilingualIDList = new ArrayList<>();
					multilingualIDList.add("IDS_DELETEBIOBANKECATALOGUEREQUESTSAMPLE");
	...
	public Boolean isProductAvailableInForm(final int nbiobankecatreqexternalcode, final int nreqformtypecode,
			final int nproductcode, final String sparentsamplecode, final UserInfo userInfo) throws Exception {
		String strQuery = "";

		if (Enumeration.RequestFormType.BIOSAMPLEBASED.getRequestFormType() == nreqformtypecode) {
			strQuery = "SELECT nproductcode FROM biobankecataloguerequestexternaldetails where nbiobankecatreqexternalcode = "
					+ nbiobankecatreqexternalcode + " and nproductcode = " + nproductcode + " and nsitecode = "
					+ userInfo.getNtranssitecode() + " and nstatus = "
					+ Enumeration.TransactionStatus.ACTIVE.gettransactionstatus();
		} else if (Enumeration.RequestFormType.PARENTSAMPLEBASED.getRequestFormType() == nreqformtypecode
				&& sparentsamplecode != null) {
			strQuery = "SELECT sparentsamplecode FROM biobankecataloguerequestexternaldetails where nbiobankecatreqexternalcode = "
					+ nbiobankecatreqexternalcode + " and nproductcode = " + nproductcode + " and sparentsamplecode = "
					+ sparentsamplecode + " and nsitecode = " + userInfo.getNtranssitecode() + " and nstatus = "
					+ Enumeration.TransactionStatus.ACTIVE.gettransactionstatus();
		}
		List<BiobankECatalogueRequestExternalDetails> lstproductcode = jdbcTemplate.query(strQuery,
				new BiobankECatalogueRequestExternalDetails());
	...
	public ResponseEntity<Map<String, Object>> getActiveSampleDetail(Map<String, Object> inputMap, UserInfo userInfo)
		...
		final Integer nThirdPartyCode = getThirdPartyCodeForUser(userInfo);
		if (nThirdPartyCode == null) {
			final String msg = commonFunction.getMultilingualMessage("IDS_USERNOTINNGSTHIRDPARTY",
					userInfo.getSlanguagefilename());
		...
		final int detailCode = Integer.parseInt(inputMap.get("nbiobankecatreqexternaldetailcode").toString());
		final String sql = "select bcd.nbiobankecatreqexternaldetailcode, bcd.nbiobankecatreqexternalcode, bc.sformnumber, "
				+ "       s.ssitename as sreceivingsitename, bc.nreceiversitecode, "
				+ "       bcd.nbioprojectcode, bp.sprojecttitle, bcd.nproductcode, "
				+ "       pr.sproductname || ' (' || pc.sproductcatname || ')' as sproductname, "
				+ "       bcd.sparentsamplecode, bcd.nreqnoofsamples, bcd.sreqminvolume, bcd.naccnoofsamples, bcd.saccminvolume, "
				+ "       bcd.dtransactiondate, bcd.ntztransactiondate, bcd.noffsetdtransactiondate, tz1.stimezoneid as stztransactiondate, "
				+ "       COALESCE(TO_CHAR(bcd.dtransactiondate,'" + userInfo.getSsitedate()
				+ "'), '') as stransactiondate "
				+ "  from biobankecataloguerequestexternaldetails bcd, biobankecataloguerequestexternal bc, site s, "
				+ "       bioproject bp, product pr, productcategory pc, timezone tz1 "
				+ " where bcd.nbiobankecatreqexternaldetailcode = " + detailCode + " " + "   and bcd.nstatus = "
				+ Enumeration.TransactionStatus.ACTIVE.gettransactionstatus() + " " + "   and bcd.nsitecode = "
				+ userInfo.getNtranssitecode() + " "
				+ "   and bc.nbiobankecatreqexternalcode = bcd.nbiobankecatreqexternalcode " + "   and bc.nstatus = "
				+ Enumeration.TransactionStatus.ACTIVE.gettransactionstatus() + " " + "   and bc.nsitecode = "
				...
				+ Enumeration.TransactionStatus.ACTIVE.gettransactionstatus() + " " + "   and s.nmastersitecode = "
				+ userInfo.getNmastersitecode() + " "
				+ "   and bcd.nbioprojectcode = bp.nbioprojectcode and bp.nstatus = "
				+ Enumeration.TransactionStatus.ACTIVE.gettransactionstatus() + " "
				+ "   and bcd.nproductcode = pr.nproductcode and pr.nsitecode = " + userInfo.getNmastersitecode() + " "
				+ "   and pr.nstatus = " + Enumeration.TransactionStatus.ACTIVE.gettransactionstatus() + " "
				+ "   and pc.nproductcatcode = pr.nproductcatcode and pc.nsitecode = " + userInfo.getNmastersitecode()
		...
		final BiobankECatalogueRequestExternalDetails dto = (BiobankECatalogueRequestExternalDetails) jdbcUtilityTemplateFunction
				.queryForObject(sql, BiobankECatalogueRequestExternalDetails.class, jdbcTemplate);

		returnMap.put("success", true);
		returnMap.put("selectedBiobankECatalogueExternalDetails", dto != null ? dto : null);
...
//		final String sallchildsitecodes = String.valueOf(userInfo.getNtranssitecode());
...
//	}
	...
	public ResponseEntity<Object> getSubjectCountsByProductAndProject(final UserInfo userInfo) throws Exception {
	    final String sallchildsitecodes = String.valueOf(userInfo.getNtranssitecode());
	    final String sbioprojectcodes = getProjectsofSite(userInfo);
	    final String siteInList = (sallchildsitecodes != null && !sallchildsitecodes.trim().isEmpty()) ? sallchildsitecodes : "-1";
	    final String projectInList = (sbioprojectcodes != null && !sbioprojectcodes.trim().isEmpty()) ? sbioprojectcodes : "-1";
	    final int active = Enumeration.TransactionStatus.ACTIVE.gettransactionstatus();
	    final int yes = Enumeration.TransactionStatus.YES.gettransactionstatus();
	    final int masterSite = userInfo.getNmastersitecode();
	    ...
	    final String cte = "WITH unique_subjects AS (" +
	        ...
	        " FROM samplestoragetransaction st JOIN biosubjectdetails bd ON bd.ssubjectid=st.ssubjectid" +
	        " AND bd.nissampleaccesable=" + yes +
	        " AND bd.nstatus=" + active +
	        " AND bd.nsitecode=" + masterSite +
	        " WHERE COALESCE(st.ssubjectid,'')<>'' AND st.nsitecode IN(" + siteInList + ")" +
	        ...
	        " AND p.nsitecode=" + masterSite +
	        " AND p.nstatus=" + active +
	        " LEFT JOIN productcategory pc ON pc.nproductcatcode=p.nproductcatcode" +
	        " AND pc.nsitecode=" + masterSite +
	        " AND pc.nstatus=" + active +
	        " JOIN bioproject bp ON bp.nbioprojectcode=us.nbioprojectcode" +
	        " AND bp.nbioprojectcode IN(" + projectInList + ")" +
	        " AND bp.nsitecode=" + masterSite +
	        " AND bp.nstatus=" + active +
	        ")";
	    final String sql = cte +
	        " SELECT sproductname,sprojecttitle,nproductcode,COUNT(DISTINCT ssubjectid) AS ndistinctsubjects" +
	        " FROM base GROUP BY sproductname,sprojecttitle,nproductcode ORDER BY sproductname,sprojecttitle";
	...
	public ResponseEntity<Object> getSubjectCountsByProductAndDisease(final UserInfo userInfo) throws Exception {
	    ...
	    final String cte = "WITH unique_subjects AS (" +
	        ...
	        " COALESCE(d.sdiseasename,dc.sdiseasecategoryname) AS sgroup" +
	        " FROM unique_subjects us JOIN product p ON p.nproductcode=us.nproductcode" +
	        " AND p.nsitecode=" + masterSite +
	        " AND p.nstatus=" + active +
	        " LEFT JOIN productcategory pc ON pc.nproductcatcode=p.nproductcatcode" +
	        ...
	        " AND bp.nstatus=" + active +
	        " JOIN diseasecategory dc ON dc.ndiseasecategorycode=bp.ndiseasecategorycode" +
	        " AND dc.nsitecode=" + masterSite +
	        " AND dc.nstatus=" + active +
	        " LEFT JOIN disease d ON d.ndiseasecode=bp.ndiseasecode" +
	        " AND d.nsitecode=" + masterSite +
	...
	public ResponseEntity<Object> getProductComboDataForSampleAdd(Map<String, Object> inputMap, UserInfo userInfo)
		...
		if (nThirdPartyCode == null) {
			...
			if (Enumeration.RequestFormType.PARENTSAMPLEBASED.getRequestFormType() == objBiobankECatalogueExternal
					.getNreqformtypecode()) {
				strQuery = "SELECT p.nproductcode, (p.sproductname || ' (' || pc.sproductcatname || ')') AS sproductname"
						...
						+ objBiobankECatalogueExternal.getNbioprojectcode() + " AND st.nsitecode = "
						+ objBiobankECatalogueExternal.getNreceiversitecode()
						+ " AND NOT EXISTS (SELECT 1 FROM biobankecataloguerequestexternaldetails d"
						+ " WHERE d.nbiobankecatreqexternalcode = " + nbiobankecatreqexternalcode
				...
				strQuery = "SELECT bp.sprojecttitle, bp.nbioprojectcode, st.nsitecode, s.ssitename, p.sproductname || ' ('|| pc.sproductcatname || ')' as sproductname, st.nproductcode, "
						...
						+ "       REPLACE(SUM(COALESCE(CASE WHEN (TRIM(st.sqty COLLATE \"default\")) ~ ("
						+ patternLiteral + ") " + "                 THEN " + castExpr
						...
						+ userInfo.getNmastersitecode() + " AND pc.nstatus = "
						+ Enumeration.TransactionStatus.ACTIVE.gettransactionstatus() + " "
						+ "LEFT JOIN biobankecataloguerequestexternaldetails bcd ON bcd.nproductcode = p.nproductcode "
						+ "  AND bcd.nbiobankecatreqexternalcode = " + nbiobankecatreqexternalcode + " "
						...
						+ objBiobankECatalogueExternal.getNreceiversitecode() + " " + "  AND st.nprojecttypecode = "
						+ objBiobankECatalogueExternal.getNbioprojectcode() + " " + "  AND bcd.nproductcode IS NULL "
	...
	public ResponseEntity<Object> getParentSamples(final Map<String, Object> inputMap, final UserInfo userInfo) {
		...
		final Integer nbiobankecatreqexternalcode = inputMap.get("nbiobankecatreqexternalcode") != null
				? Integer.valueOf(inputMap.get("nbiobankecatreqexternalcode").toString().trim())
				: -1;
		...
		final Integer nbiobankecatreqexternaldetailcode = inputMap.get("nbiobankecatreqexternaldetailcode") != null
				? Integer.valueOf(inputMap.get("nbiobankecatreqexternaldetailcode").toString().trim())
				: -1;
		...
		final String sql = "SELECT TRIM(st.sparentsamplecode) AS sparentsamplecode, "
				...
				+ "   AND COALESCE(TRIM(st.ssubjectid),'') <> '' " + "   AND st.nprojecttypecode = " + nbioprojectcode
				+ " " + "   AND st.nsitecode        = " + nsitecode + " " + "   AND st.nproductcode     = "
				+ nproductcode + " " + "   AND st.nstatus          = " + active + " "
				+ "   AND (CASE WHEN (TRIM(st.sqty COLLATE \"default\")) ~ (" + pattern + ") " + "             THEN "
				+ castExprSt + " ELSE NULL END) " + "       >= (CASE WHEN (TRIM('" + sreqminvolume
				+ "' COLLATE \"default\")) ~ (" + pattern + ") " + "                THEN " + castExprReq
				+ " ELSE NULL END) " + "   AND NOT EXISTS ( "
				+ "       SELECT 1 FROM biobankecataloguerequestexternaldetails d "
				+ "        WHERE d.nbiobankecatreqexternalcode = " + nbiobankecatreqexternalcode + " "
				...
				+ "               OR d.nbiobankecatreqexternaldetailcode <> " + nbiobankecatreqexternaldetailcode + " "
	...
	private Integer getThirdPartyCodeForUser(final UserInfo userInfo) {

		final String sql = "SELECT t.nthirdpartycode " + "  FROM thirdparty t " + "  JOIN thirdpartyusermapping tu "
				+ "    ON tu.nthirdpartycode = t.nthirdpartycode " + "   AND t.nstatus = "
				...
				+ userInfo.getNmastersitecode() + "   AND tu.nsitecode = " + userInfo.getNmastersitecode()
				+ " AND t.nisngs = " + Enumeration.TransactionStatus.YES.gettransactionstatus()
				+ " AND tu.nusercode = " + userInfo.getNusercode()
				+ " JOIN userroleconfig urc ON urc.nuserrolecode = tu.nuserrolecode AND urc.nneedngsflow = "+ Enumeration.TransactionStatus.YES.gettransactionstatus()
				+ " and urc.nstatus = "+ Enumeration.TransactionStatus.ACTIVE.gettransactionstatus() 
				+ " and urc.nsitecode = "+userInfo.getNmastersitecode()
				+ " and urc.nuserrolecode = "+userInfo.getNuserrole();
