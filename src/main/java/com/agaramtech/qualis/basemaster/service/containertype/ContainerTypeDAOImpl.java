...
public class ContainerTypeDAOImpl implements ContainerTypeDAO {
	 ...
	 */
	@Override
	public ResponseEntity<Object> getContainerType(final UserInfo userInfo) throws Exception {
		final String strQuery = "select c.*,coalesce(ts.jsondata->'stransdisplaystatus'->>'"+userInfo.getSlanguagetypecode()+"',ts.jsondata->'stransdisplaystatus'->>'en-US') as stransdisplaystatus  from containertype c,transactionstatus ts"
				+ " where ts.ntranscode=c.nsamplecontainer and c.nstatus = "+ Enumeration.TransactionStatus.ACTIVE.gettransactionstatus() + " and ncontainertypecode > 0 "
				+ " and c.nsitecode = " + userInfo.getNmastersitecode() + " order by ncontainertypecode";
		LOGGER.info("Get Container Type Qry :--->"+strQuery);
		return new ResponseEntity<>(jdbcTemplate.query(strQuery, new ContainerType()),
				HttpStatus.OK);
	}
	 ...
	 */
	@Override
	public ResponseEntity<Object> createContainerType(final ContainerType containerType, final UserInfo userInfo) throws Exception {
		...
		if (ContainerTypeByName == null) {
			...
			nSeqNo++;
			final String containerTypeInsert = "insert into containertype(ncontainertypecode,scontainertype,sdescription,nsamplecontainer,dmodifieddate,nsitecode,nstatus)"
					+ " values(" + nSeqNo + ",N'"
					+ stringUtilityFunction.replaceQuote(containerType.getScontainertype()) + "',N'"
					+ stringUtilityFunction.replaceQuote(containerType.getSdescription()) + "',"+containerType.getNsamplecontainer()+""
			...
			return getContainerType(userInfo);
		} else {
	 ...
	 */
	@Override
	public ResponseEntity<Object> updateContainerType(final ContainerType containerType, final UserInfo userInfo) throws Exception {
		...
		if (objContainerType == null) {
			...
			final List<ContainerType> containertypeList = jdbcTemplate.query(queryString,
					new ContainerType());
			if (containertypeList.isEmpty()) {
				final String updateQueryString = "update containertype set scontainertype='"
						...
						+ stringUtilityFunction.replaceQuote(containerType.getSdescription()) + "',nsamplecontainer="+containerType.getNsamplecontainer()+",dmodifieddate='"
	 ...
	 */
	@Override
	public ContainerType getActiveContainerTypeById(final int ncontainertypecode, final UserInfo userInfo) throws Exception {
		
		final String strQuery = "select c.*,coalesce(ts.jsondata->'stransdisplaystatus'->>'en-US',ts.jsondata->'stransdisplaystatus'->>'en-US') as stransdisplaystatus  from containertype c,transactionstatus ts"
				+ " where ts.ntranscode=c.nsamplecontainer and c.ncontainertypecode = "
				+ ncontainertypecode + "";
		return (ContainerType) jdbcUtilityFunction.queryForObject(strQuery, ContainerType.class, jdbcTemplate);
	}
	 ...
	 */
	@Override
	public ResponseEntity<Object> deleteContainerType(final ContainerType containerType, final UserInfo userInfo) throws Exception {
		final ContainerType containerTypebyID = getActiveContainerTypeById(
