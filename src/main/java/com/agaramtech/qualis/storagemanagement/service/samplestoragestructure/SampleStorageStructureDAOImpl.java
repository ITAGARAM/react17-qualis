...
public class SampleStorageStructureDAOImpl implements SampleStorageStructureDAO {
	...
	public ResponseEntity<Object> getSampleStorageStructure(final int nsampleStorageLocationCode,
		...
		if (filterStorageCategory != null && filterStorageCategory.size() > 0) {
			...
			if (nstorageCategoryCode == 0) {
				...
				sQuery = "select ssl.*, s.ssitename from samplestoragelocation ssl, site s "
						+ " where ssl.nsitecode =s.nsitecode "
			...
			if (list.size() > 0) {
				...
				outputMap.putAll((Map<String, Object>) sampleStorageCommons
						.getSelectedSampleStorageLocation(nlocationCode, nsamplestorageversioncode, userInfo)
						.getBody());
			} else {
	...
	public ResponseEntity<? extends Object> getSelectedSampleStorageStructure(final int nsampleStorageLocationCode,
		...
		return sampleStorageCommons.getSelectedSampleStorageLocation(nsampleStorageLocationCode,
	...
	private ResponseEntity<Object> createSampleStorageVersion(final int nsampleStorageLocationCode,
		...
		insertQueryString = " insert into samplestorageversion (nsamplestorageversioncode, nsamplestoragelocationcode, nversionno, napprovalstatus, jsondata, nstatus, nsitecode, dmodifieddate) values ( "
				...
				+ stringUtilityFunction.replaceQuote(jsonObject.toString()) + "',1, " + userInfo.getNmastersitecode()
				+ ", '" + dateUtilityFunction.getCurrentDateTime(userInfo) + "' )";
	...
	public ResponseEntity<Object> approveSampleStorageVersion(final SampleStorageStructure sampleStorageLocation,
		...
		if (versionApprovalStatus.getNapprovalstatus() == Enumeration.TransactionStatus.APPROVED
			...
			if (previousVersionApproval != null) {
				...
				//start
				insertQueryString += " delete from samplestoragemapping "
						+ " where nsamplestoragecontainerpathcode in(select nsamplestoragecontainerpathcode from samplestoragecontainerpath "
						+ " where nsamplestorageversioncode=" + previousVersionApproval.getNsamplestorageversioncode()
						+ ");";
				
				insertQueryString += " delete from  storageinstrument " + " where nsamplestorageversioncode="
						+ previousVersionApproval.getNsamplestorageversioncode() + ";";
	
				
				insertQueryString += "  delete from samplestoragecontainerpath " + " where nsamplestorageversioncode="
						+ previousVersionApproval.getNsamplestorageversioncode() + ";";
				//end
			...
			final String str=" select * from storageinstrument where nsamplestoragelocationcode ="+sampleStorageLocation.getNsamplestoragelocationcode()+" "
					+ " and nstatus="+Enumeration.TransactionStatus.ACTIVE.gettransactionstatus()+"";
			
			final List<StorageInstrument> lstStorageInstrument = jdbcTemplate.query(str,
					new StorageInstrument());
			String stringstorageIns="";
			 String updatequery ="";
			if(lstStorageInstrument.isEmpty()) {
				nsequenceno++;
				stringstorageIns = "INSERT INTO storageinstrument "
				        + "(nstorageinstrumentcode, nstoragecategorycode, nsamplestoragelocationcode, nsamplestorageversioncode, "
				        + "nstorageconditioncode, ninstrumentcatcode, ninstrumentcode, nregionalsitecode, dmodifieddate, "
				        + "nsitecode, nstatus) "
				        + "VALUES ("
				        + nsequenceno + ", "
				        +  sampleStorageLocation.getNstoragecategorycode()+ ", "
				        + sampleStorageLocation.getNsamplestoragelocationcode() + ", "
				        + sampleStorageVersion.getNsamplestorageversioncode()+ ", "
				        + Enumeration.TransactionStatus.NA.gettransactionstatus() + ", "
				        + Enumeration.TransactionStatus.NA.gettransactionstatus() + ", "
				        + Enumeration.TransactionStatus.NA.gettransactionstatus() + ", "
				        + deleteSampleStorageLocation.getNsitecode()+ ", "
				        + "'" + dateUtilityFunction.getCurrentDateTime(userInfo) + "', "
				        + userInfo.getNmastersitecode() + ", "
				        + Enumeration.TransactionStatus.ACTIVE.gettransactionstatus()
				        + ");";
			  ...
			  updatequery= "update seqnostoragemanagement set nsequenceno ="+nsequenceno+" where stablename ='storageinstrument' and nstatus="
					+ Enumeration.TransactionStatus.ACTIVE.gettransactionstatus() + ";";
			}else {
				stringstorageIns=" update storageinstrument set nsamplestorageversioncode="+sampleStorageVersion.getNsamplestorageversioncode()+""
						+" where nsamplestoragelocationcode="+sampleStorageLocation.getNsamplestoragelocationcode()+";";
			...
			jdbcTemplate.execute(insertQueryString + updatequery + stringstorageIns);
			...
			final String sSetting = "select ssettingvalue from settings where nsettingcode ="+Enumeration.Settings.INSTRUMENT_BASED_STORAGE.getNsettingcode()+" and nstatus = "
					+ Enumeration.TransactionStatus.ACTIVE.gettransactionstatus() + ";";
			final String sNeedInstrumentFlow = (String) jdbcUtilityFunction.queryForObject(sSetting, String.class, jdbcTemplate);
			final int nNeedInstrumentFlow = Integer.parseInt(sNeedInstrumentFlow);
			...
			if (inputMap.get("containerpath") != null) {
				...
				String insertQuery = " insert into samplestoragecontainerpath(nsamplestoragecontainerpathcode,nsamplestoragelocationcode,nsamplestorageversioncode,"
						+ "jsondata,scontainerlastnode,dmodifieddate,nsitecode,nstatus,suid) " + " select generate_series("
						...
						+ "' as dmodifieddate, " + Enumeration.TransactionStatus.NA.gettransactionstatus() +  " as nsitecode, "
						+ Enumeration.TransactionStatus.ACTIVE.gettransactionstatus() + " as nstatus, "
						+ " json_array_elements('" + inputMap.get("containerid") + "')#>>'{}' as suid; ";
				...
				insertQuery += " CREATE TABLE IF NOT EXISTS  samplestoragetransaction_" + (nprojecttypecode == -1 ? 0 : nprojecttypecode)
						+ " PARTITION OF samplestoragetransaction   FOR VALUES IN (" + nprojecttypecode
						+ ");";
				...
				if ((int) ((Map<String, Object>) inputMap.get("selectedSampleStorageLocation"))
						.get("nneedautomapping") == Enumeration.TransactionStatus.YES.gettransactionstatus() && nNeedInstrumentFlow==Enumeration.TransactionStatus.NO.gettransactionstatus()) {
					...
					final int nsitecode = deleteSampleStorageLocation.getNsitecode();
					outputMap.put("nsitecode",nsitecode);
					//ALPDJ21-33--Added by Vignesh(05-08-2025)-->Storage Instrument - Screen Development
					outputMap.put("nstorageinstrumentcode",nsequenceno);
					
				//ALPDJ21-71--Added by Vignesh(06-09-2025)--new fields added the for the partition of sample storage mapping
					outputMap.put("nNeedInstrumentFlow",nNeedInstrumentFlow);
					outputMap.put("npartitioncode",inputMap.get("nprojecttypecode"));
...
//				+ userInfo.getNtranssitecode();
	...
	*/
	...
	/**
	 * This method is used to checks the Previous version is used in Storage Instrument.
	 *
	 * @param inputMap [Map] map object with userInfo [UserInfo] holding logged in
	 *                 user details and nmasterSiteCode [int] primary key of site
	 *                 object for which the list is to be fetched Input :
	 *                 {"nsamplestoragelocationcode":1,"userinfo":{nmastersitecode":
	 *                 -1,slanguagetypecode:"en-US",nsamplestoragelocationcode:3,nsamplestorageversioncode:42},nsamplestorageversioncode:1}
	 * @return response entity object holding response status 
	 * @throws Exception exception
	 */
	@Override
	public ResponseEntity<Object> checkStorageInstrumentVersion(final Map<String, Object> inputMap,
			final UserInfo userInfo) throws Exception {

		boolean needConfirmMessage = false;
		int nsamplestoragelocationcode = -1;
		...
		final String strcheck = "select sl.* from samplestoragelocation sl, samplestorageversion sv, "
				...
				+ " and ssc.nsamplestoragecontainerpathcode=ssm.nsamplestoragecontainerpathcode "
				+ " and sst.nsamplestoragemappingcode=ssm.nsamplestoragemappingcode "
				+ " and sl.nsamplestoragelocationcode=" + nsamplestoragelocationcode
		...
		if (lstcheck.size() > 0) {
			...
			final String str = " select * from storageinstrument where  nsamplestorageversioncode=(select nsamplestorageversioncode from samplestorageversion "
					+ " where napprovalstatus=" + Enumeration.TransactionStatus.APPROVED.gettransactionstatus()
					+ " and nsamplestoragelocationcode=" + nsamplestoragelocationcode + ")" + " and nstatus="
					+ Enumeration.TransactionStatus.ACTIVE.gettransactionstatus() + " "
					+ " and nsitecode=" + userInfo.getNmastersitecode()+"";

			final List<StorageInstrument> lstStorageInstrument = jdbcTemplate.query(str, new StorageInstrument());

			if (!lstStorageInstrument.isEmpty()) {
				needConfirmMessage = true;
			}
			responseMap.put("needConfirmMessage", needConfirmMessage);
