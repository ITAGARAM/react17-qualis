...
/**
 * This class is used to perform CRUD Operation on "samplestoragetransacation"
...
public class SampleStorageMoveDAOImpl implements SampleStorageMoveDAO {
	...
	public ResponseEntity<Map<String, Object>> getsamplestoragemove(final Map<String, Object> inputMap,
		...
		String strQuery = " select ssl.*,ts.jsondata->'stransdisplaystatus'->>'" + userInfo.getSlanguagetypecode()
				...
				+ " and nsitecode=" + userInfo.getNmastersitecode() + ") and  ts.ntranscode=ssl.nmappingtranscode "
...
//		outputMap.put("sampleType", list);
		...
		if (nNeedInstrumentFlow == Enumeration.TransactionStatus.YES.gettransactionstatus()) {
			...
			int ninstrumentcode = listInstument.isEmpty() ? Enumeration.TransactionStatus.NA.gettransactionstatus()
					: (int) listInstument.getLast().get("ninstrumentcode");

			final Map<String, Object> map = new HashMap<>();

			map.put("label", "sampleStoragetransaction");
			map.put("valuemember", "nsamplestoragemappingcode");
			map.put("filterquery",
					" nprojecttypecode = " + nprojecttypecode + " and ninstrumentcode = " + ninstrumentcode + "   ");
			map.put("source", "view_samplestoragelocation");

			final var response = getDynamicFilterExecuteData(map, userInfo);
...
//		outputMap.put("projectType", list);
	 ...
	 * Enumeration.TransactionStatus.APPROVED.gettransactionstatus() +
	 * " and nsamplestoragelocationcode=" + nsamplestoragelocationcode +
	 * ") and nsamplestoragecontainerpathcode not in " +
	 ...
	 * " and cts.ncontainertypecode=c.ncontainertypecode " +
	 * " group by c.ncontainertypecode order by c.ncontainertypecode"; final
	 * List<ContainerType> lstcontainerType = jdbcTemplate.query(containertypeQuery,
	 * new ContainerType());
	 * 
	 ...
	 * nsamplestoragelocationcode); outputMap.putAll(getsamplestoragemove(outputMap,
	 * userInfo).getBody()); return new ResponseEntity<Object>(outputMap,
	 * HttpStatus.OK); }
	 * 
	 * @Override public ResponseEntity<Object> createSampleStorageTransaction(final
	 * Map<String, Object> inputMap, final UserInfo userInfo) throws Exception {
	 * final String sQuery = " lock  table locksamplestoragetransaction " +
	 * Enumeration.ReturnStatus.TABLOCK.getreturnstatus();
	 * jdbcTemplate.execute(sQuery); String Query = ""; String updateQuery = "";
	 * Map<String, Object> barcodeDetails = new HashMap<>(); boolean checkValue =
	 * false;
	 * 
	 * if ((int) inputMap.get("nbarcodedescription") ==
	 * Enumeration.TransactionStatus.YES.gettransactionstatus()) { barcodeDetails =
	 * readBarcode(inputMap, userInfo); inputMap.put("jsondata",
	 * barcodeDetails.get("jsonValue")); } try { final String strCheckPosition =
	 * "select * from samplestoragetransaction where " +
	 * " nsamplestoragelocationcode=" + inputMap.get("nsamplestoragelocationcode") +
	 * " and nsamplestoragemappingcode=" + inputMap.get("nsamplestoragemappingcode")
	 * + " and sposition='" + inputMap.get("sposition") + "' and nsitecode=" +
	 * userInfo.getNtranssitecode() + " and nstatus=" +
	 * Enumeration.TransactionStatus.ACTIVE.gettransactionstatus();
	 * jdbcTemplate.execute(strCheckPosition); checkValue = false;
	 * inputMap.put("isAlreadyExists", true); } catch (final Exception e) {
	 * checkValue = true; }
	 * 
	 * if (checkValue == true) { final String strCheckValue =
	 * "select * from samplestoragetransaction where spositionvalue='" +
	 * inputMap.get("spositionvalue") + "' and nsitecode=" +
	 * userInfo.getNtranssitecode() + " and nstatus=" +
	 * Enumeration.TransactionStatus.ACTIVE.gettransactionstatus(); final
	 * List<Map<String, Object>> lstCheck =
	 * jdbcTemplate.queryForList(strCheckValue); if (lstCheck.isEmpty()) {
	 * 
	 * final String sectSeq =
	 * "select nsequenceno from seqnobasemaster where stablename='samplestoragetransaction' and nstaus="
	 * + Enumeration.TransactionStatus.ACTIVE.gettransactionstatus(); final Integer
	 * objseq = jdbcTemplate.queryForObject(sectSeq, Integer.class);
	 * 
	 * final int nsamplestoragetransactioncode = objseq + 1; Query =
	 * " insert into samplestoragetransaction (nsamplestoragetransactioncode, nsamplestoragelocationcode, nsamplestoragemappingcode, nprojecttypecode, sposition,"
	 * +
	 ...
	 * userInfo.getNtranssitecode() + "," +
	 * Enumeration.TransactionStatus.ACTIVE.gettransactionstatus() + ") ;";
	 * updateQuery = " update seqnobasemaster set nsequenceno= " +
	 * nsamplestoragetransactioncode +
	 * " where stablename='samplestoragetransaction';"; jdbcTemplate.execute(Query +
	 * updateQuery); final List<String> multilingualIDList = new ArrayList<>();
	 * final List<Object> savedTestList = new ArrayList<>();
	 * 
	 * final SampleStorageTransaction samplestoragetransaction = new
	 * SampleStorageTransaction();
	 * 
	 * final Integer nsamplestoragelocationcodeint = (Integer)
	 * inputMap.get("nsamplestoragelocationcode"); final Short
	 * nsamplestoragelocationcodeintshort =
	 * nsamplestoragelocationcodeint.shortValue();
	 * 
	 * samplestoragetransaction.setNsamplestoragelocationcode(
	 * nsamplestoragelocationcodeintshort);
	 * samplestoragetransaction.setNsamplestoragetransactioncode(
	 * nsamplestoragetransactioncode);
	 * samplestoragetransaction.setNsamplestoragemappingcode((int)
	 * inputMap.get("nsamplestoragemappingcode"));
	 * samplestoragetransaction.setSposition(inputMap.get("sposition").toString());
	 * samplestoragetransaction.setSpositionvalue(inputMap.get("spositionvalue").
	 * toString()); samplestoragetransaction.setDmodifieddate(dateUtilityFunction.
	 * getCurrentDateTime(userInfo));
	 * samplestoragetransaction.setNsitecode(userInfo.getNtranssitecode());
	 * samplestoragetransaction .setNstatus((short)
	 * Enumeration.TransactionStatus.ACTIVE.gettransactionstatus());
	 * 
	 * savedTestList.add(samplestoragetransaction);
	 * multilingualIDList.add("IDS_STORAGESTRUCTURENAME");
	 * auditUtilityFunction.fnInsertAuditAction(savedTestList, 1, null,
	 * multilingualIDList, userInfo);
	 * 
	 * } else { return new ResponseEntity<>(
	 * commonFunction.getMultilingualMessage("IDS_ALREADYEXISTS",
	 * userInfo.getSlanguagefilename()), HttpStatus.CONFLICT); } } return
	 * getSheetData(inputMap, userInfo); }
	 * 
	 * private ResponseEntity<Object> getSheetData(final Map<String, Object>
	 * inputMap, final UserInfo userInfo) { final Map<String, Object> outputMap =
	 * new HashMap<>(); Map<String, Object> map = new HashMap<>(); String strQuery =
	 * "select *, jsondata  as \"additionalInfo\" from samplestoragetransaction where nsamplestoragelocationcode="
	 * + inputMap.get("nsamplestoragelocationcode") +
	 * " and nsamplestoragemappingcode=" + inputMap.get("nsamplestoragemappingcode")
	 * + " and sposition='" + inputMap.get("sposition") + "' and npositionfilled=" +
	 * Enumeration.TransactionStatus.YES.gettransactionstatus() + " and nstatus=" +
	 * Enumeration.TransactionStatus.ACTIVE.gettransactionstatus() + ";";
	 * 
	 * try { map = jdbcTemplate.queryForMap(strQuery); } catch (final Exception e) {
	 * map = new HashMap<>(); } outputMap.put("cellData", map);
	 * 
	 * strQuery =
	 * "(select ssm.nnoofcontainer-count(st.sposition) as navailablespace from samplestoragetransaction st ,samplestoragemapping ssm "
	 * + "	where   ssm.nsamplestoragemappingcode=st.nsamplestoragemappingcode " +
	 * " and st.nsamplestoragemappingcode= " +
	 * inputMap.get("nsamplestoragemappingcode") + " and st.nstatus=" +
	 * Enumeration.TransactionStatus.ACTIVE.gettransactionstatus() +
	 * " and st.spositionvalue!='' " + " group by ssm.nnoofcontainer " + "		)";
	 * 
	 * try { map = jdbcTemplate.queryForMap(strQuery); } catch (final Exception e) {
	 * map = new HashMap<>(); } outputMap.put("navailablespace", map);
	 * 
	 ...
	 *                 fetched,nsourcemappingcode:Indicates the primary key or value
	 *                 field used for identifying the record from the source
	 *                 container mapping code,
	 *                 nsourcesamplestoragelocationcode:Indicates the primary key or
	 *                 value field used for identifying the record from the source
	 *                 container structure code, nsourceprojecttypecode:Indicates
	 *                 the primary key or value field used for identifying the
	 *                 record from the source project type code,
	 *                 nsamplestoragelocationcode:Indicates the primary key or value
	 *                 field used for identifying the record to be transferred to
	 *                 the source sample storage location code,
	 *                 nsamplestoragemappingcode:Indicates the primary key or value
	 *                 field used for identifying the record to be transferred to
	 *                 the source sample storage mapping code,
	 *                 nprojecttypecode:Indicates the primary key or value field
	 *                 used for identifying the record to be transferred to the
	 *                 source project type code,
	 *                 ssamplestoragelocationname:Indicates the primary key or value
	 *                 field used for identifying from the source Sample Storage
	 *                 location Name, ssamplestoragepathname:Indicates the primary
	 *                 key or value field used for identifying from the source
	 *                 Sample Storage location Path,
	 *                 stosamplestoragelocationname:Indicates the primary key or
	 *                 value field used for identifying the record to be transferred
	 *                 to the Sample Storage Location Name,
	 *                 stosamplestoragepathname:Indicates the primary key or value
	 *                 field used for identifying the record to be transferred to
	 *                 the Sample Storage Path,
	 * 
	 *                 Input : {"userinfo":{nmastersitecode":
	 *                 -1},"nsourcemappingcode":3,"nsourcesamplestoragelocationcode":2,"nsourceprojecttypecode":1,"nsamplestoragelocationcode":1,"nsamplestoragemappingcode":1,"nprojecttypecode":1,"ssamplestoragelocationname":"FZ02",
	 *                 "ssamplestoragepathname":"root > Label > Label >
	 *                 Label","stosamplestoragelocationname":"FZ01","stosamplestoragepathname":"FZ
	 *                 > Rack > Tray > Cell Position"}
	 * 
	 * @return response entity object holding response status and list of available
	 *         Container Location.
	 * 
	...
	public ResponseEntity<Object> updateSampleStorageTransaction(final Map<String, Object> inputMap,
		...
		final String str = "select ssettingvalue from settings where nsettingcode=84 and nstatus = "
				+ Enumeration.TransactionStatus.ACTIVE.gettransactionstatus();
		final Settings objSettings = (Settings) jdbcUtilityFunction.queryForObject(str, Settings.class, jdbcTemplate);
		final int nIstrumentBasedNeed = Integer.parseInt(objSettings.getSsettingvalue());
		...
		if (inputMap.containsKey("isMultiContainermove")) {
			...
			for (int i = 0; i < lstsourcePath.size(); i++) {
				...
				if (mapobj.isPresent()) {
					...
					updateQuery += " UPDATE public.samplestoragetransaction SET   " + " nsamplestoragelocationcode= "
							...
							+ mapobj.get().get("nsamplestoragemappingcode") + ",dmodifieddate='"
							+ dateUtilityFunction.getCurrentDateTime(userInfo)
			...
			if (nIstrumentBasedNeed == Enumeration.TransactionStatus.NO.gettransactionstatus()) {
				updateQuery = " UPDATE public.samplestoragetransaction SET   " + " nsamplestoragelocationcode= "
						+ inputMap.get("nsamplestoragelocationcode") + " ,nsamplestoragemappingcode="
						+ inputMap.get("nsamplestoragemappingcode") + ",dmodifieddate='"
						+ dateUtilityFunction.getCurrentDateTime(userInfo) + "'"
						// ALPD-5097-Comment by Vignesh R(12-06-2025)-->Sample info fields showing
						// wrongly in sample box, which is configured in barcode formatting
						...
						+ inputMap.get("nsourcemappingcode") + " " + " and nprojecttypecode="
						+ inputMap.get("nsourceprojecttypecode") + " " + " and nsamplestoragelocationcode="
						+ inputMap.get("nsourcesamplestoragelocationcode") + ") " + " and nprojecttypecode="
						+ inputMap.get("nsourceprojecttypecode") + " " + " " + " and nsamplestoragelocationcode="
						+ inputMap.get("nsourcesamplestoragelocationcode") + "";
			} else {
				updateQuery = " UPDATE public.samplestoragetransaction SET   " + " nsamplestoragelocationcode= "
						...
						+ " WHERE nsamplestoragetransactioncode "
						+ " in ( select nsamplestoragetransactioncode from samplestoragetransaction where nsamplestoragemappingcode = "
						+ inputMap.get("nsourcemappingcode") + " " + " and nsitecode=" + userInfo.getNtranssitecode()
...
//				+ " + rank() over(order by t.nsamplestoragetransactioncode)), " + userInfo.getNformcode()
//				+ ",t.nsamplestoragetransactioncode,'nsamplestoragetransactioncode','samplestoragetransaction',t.sposition,"
//				+ Enumeration.TransactionStatus.STORED.gettransactionstatus() + "," + userInfo.getNusercode() + ","
...
//				+ commonFunction.getMultilingualMessage("IDS_PROJECTYPE", userInfo.getSlanguagefilename())
//				+ " [' || coalesce(p.sprojecttitle,'') || '] ' || '"
//				+ commonFunction.getMultilingualMessage("IDS_PRODUCTNAME", userInfo.getSlanguagefilename())
//				+ " [' || coalesce(pr.sproductname,'') || ']'," + userInfo.getNtranssitecode() + ","
//				+ Enumeration.TransactionStatus.ACTIVE.gettransactionstatus() + " from samplestoragetransaction t "
//				+ " left join bioproject p on p.nbioprojectcode=t.nprojecttypecode "
//				+ " left join product pr on pr.nproductcode=t.nproductcode " + " where t.nsamplestoragelocationcode = "
//				+ inputMap.get("nsamplestoragelocationcode") + " and t.dmodifieddate = '"
//				+ dateUtilityFunction.getCurrentDateTime(userInfo) + "'" + " and t.nsitecode = "
//				+ userInfo.getNtranssitecode() + " and t.nstatus = "
//				+ Enumeration.TransactionStatus.ACTIVE.gettransactionstatus() + ";";
//
...
//				+ "where nsamplestoragelocationcode = " + inputMap.get("nsamplestoragelocationcode")
//				+ " and dmodifieddate = '" + dateUtilityFunction.getCurrentDateTime(userInfo) + "' and nsitecode="
//				+ userInfo.getNtranssitecode() + " and nstatus="
//				+ Enumeration.TransactionStatus.ACTIVE.gettransactionstatus()
...
//		jdbcTemplate.execute(strSeqUpdateCOC);
	...
	/*
	 * @Override public ResponseEntity<Object> updateSampleStorageMapping(final
	 * Map<String, Object> inputMap, final UserInfo userInfo) throws Exception {
	 * String updateQuery = ""; final Map<String, Object> mapObj = new
	 * HashMap<String, Object>(); final String strcheck =
	 * "select count(nsamplestoragemappingcode) from samplestoragemapping ssm,samplestoragecontainerpath ssc "
	 * + "where ssm.sboxid=N'" +
	 * stringUtilityFunction.replaceQuote(inputMap.get("sboxid").toString()) + "'  "
	 * + " and ssm.nsamplestoragemappingcode <> " +
	 * inputMap.get("nsamplestoragemappingcode") +
	 * "and ssc.nsamplestoragecontainerpathcode=ssm.nsamplestoragecontainerpathcode "
	 * + "and ssc.nsamplestoragelocationcode=" +
	 * inputMap.get("nsamplestoragelocationcode") + " and ssc.nsitecode=" +
	 * userInfo.getNmastersitecode() + " and ssm.nsitecode=" +
	 * userInfo.getNmastersitecode() + " and ssc.nstatus=" +
	 * Enumeration.TransactionStatus.ACTIVE.gettransactionstatus() +
	 * " and ssm.nstatus=" +
	 * Enumeration.TransactionStatus.ACTIVE.gettransactionstatus();
	 * 
	 * Integer check = null; boolean isExist = false; try { check =
	 * jdbcTemplate.queryForObject(strcheck, Integer.class); } catch (final
	 * Exception e) { check = null; } if (check == 0) { final int
	 * nsamplestoragemappingcode = (int) inputMap.get("nsamplestoragemappingcode");
	 * updateQuery = " UPDATE public.samplestoragemapping SET   sboxid='" +
	 * stringUtilityFunction.replaceQuote(inputMap.get("sboxid").toString()) +
	 * "'	WHERE nsamplestoragemappingcode=" + nsamplestoragemappingcode + ";";
	 * jdbcTemplate.execute(updateQuery); isExist = false; mapObj.put("sboxid",
	 * inputMap.get("sboxid").toString()); } else { final String strGetQuery =
	 * "select sboxid from samplestoragemapping where nsamplestoragemappingcode=" +
	 * inputMap.get("nsamplestoragemappingcode") + " and nstatus =" +
	 * Enumeration.TransactionStatus.ACTIVE.gettransactionstatus(); final String
	 * boxValue = jdbcTemplate.queryForObject(strGetQuery, String.class);
	 * mapObj.put("sboxid", boxValue); isExist = true; } mapObj.put("existCheck",
	 * isExist); return new ResponseEntity<>(mapObj, HttpStatus.OK); }
	 * 
	 * @Override public ResponseEntity<Map<String, Object>>
	 ...
	 * 
	 * jdbcTemplate.execute(updateQuery); return getsamplestoragemove(inputMap,
	 * userInfo); }
	 * 
	 ...
	 * 
	 * @Override public ResponseEntity<Object>
	 * getsamplestoragemappingSheetData(final Map<String, Object> inputMap, final
	 * UserInfo userInfo) throws Exception { final Map<String, Object> outputMap =
	 * new HashMap<>(); Map<String, Object> map = new HashMap<>();
	 * 
	 * final String nsamplestoragemappingcode = (String)
	 * inputMap.get("nsamplestoragemappingcode");
	 * 
	 * String strQuery = "";
	 * 
	 * if ((boolean) inputMap.get("isMultiSampleAdd")) { strQuery =
	 * "select jsonb_object_agg(x.nsamplestoragemappingcode,x.jsondata)   from " +
	 ...
	 * "from  samplestoragetransaction where nsamplestoragemappingcode in (" +
	 * nsamplestoragemappingcode + ")  and npositionfilled= " +
	 * Enumeration.TransactionStatus.YES.gettransactionstatus() + " and nstatus=" +
	 * Enumeration.TransactionStatus.ACTIVE.gettransactionstatus() +
	 * " group by nsamplestoragemappingcode)x"; } else { strQuery =
	 * "select jsonb_object_agg( sposition,jsonb_build_object('spositionvalue',spositionvalue, "
	 * + " 'additionalInfo', jsondata ) ) as jsondata " +
	 * " from samplestoragetransaction where nsamplestoragemappingcode=" +
	 * nsamplestoragemappingcode + " and npositionfilled= " +
	 * Enumeration.TransactionStatus.YES.gettransactionstatus() + " and nstatus=" +
	 * Enumeration.TransactionStatus.ACTIVE.gettransactionstatus(); }
	 * System.out.println(strQuery); String strMaterial =
	 * jdbcTemplate.queryForObject(strQuery, String.class);
	 * outputMap.put("sheetData", strMaterial); strQuery =
	 * " SELECT Jsonb_object_agg(x.nsamplestoragemappingcode, x.sboxid) " +
	 ...
	 * " ) and nstatus=" +
	 * Enumeration.TransactionStatus.ACTIVE.gettransactionstatus() +
	 * " GROUP  BY nsamplestoragemappingcode, sboxid)x "; strMaterial =
	 * jdbcTemplate.queryForObject(strQuery, String.class);
	 ...
	 *                 stosamplestoragepathname:Indicates the primary key or value
	 *                 field used for identifying the record to be transferred to
	 *                 the Sample Storage Path, source:Indicates the table name of
	 *                 sample storage transaction. valuemember:Indicates the primary
	 *                 key of sample storage transaction code. label:Indicates the
	 *                 label of sample storage transaction.
	 * 
	 *                 Input : {"userinfo":{nmastersitecode":
	 *                 -1},"nsourcemappingcode":3,"nsourcesamplestoragelocationcode":2,"nsourceprojecttypecode":1,"nsamplestoragelocationcode":1,"nsamplestoragemappingcode":1,"nprojecttypecode":1,"ssamplestoragelocationname":"FZ02",
	 *                 "ssamplestoragepathname":"root > Label > Label >
	 *                 Label","stosamplestoragelocationname":"FZ01","stosamplestoragepathname":"FZ
	 *                 > Rack > Tray > Cell
	 *                 Position","source":"view_samplestoragelocation","valuemember":"nsamplestoragetransactioncode","label":"sampleStoragetransaction"}
	 * 
	 * @return response entity object holding response status and list of available
	 *         available Sample Storage Data.
	 * 
	 * @throws Exception exception
	 */
	...
	public ResponseEntity<Map<String, Object>> getDynamicFilterExecuteData(final Map<String, Object> inputMap,
		...
		if (fields.contains(inputMap.get("valuemember").toString())) {
			getJSONKeysQuery = "select " + tableName + ".* from " + tableName + " where nsitecode="
					+ userInfo.getNtranssitecode() + " and nstatus ="
	 ...
	 *                 user details and nmasterSiteCode [int] primary key of site
	 *                 object for which the list is to be
	 *                 fetched,spositionvalue:Indicates the value field used for
	 *                 identifying the record of barcode id,
	 *                 nprojecttypecode:Indicates the primary key or value field
	 *                 used for identifying the record to be transferred to the
	 *                 source project type code,
	 * 
	 *                 Input : {"userinfo":{nmastersitecode":
	 *                 -1},"spositionvalue":3,"P1001156009234":2,"nprojecttypecode":1}
	 * 
	 * @return response entity object holding response status and list of available
	 *         barcode id based on the project type.
	 * 
	 * @throws Exception exception
	 */
	...
	/**
	 * This method is used to retrieve the list of available Storage Structure and
	 * Sample Storage Container Path.
	 * 
	 * @param inputMap [Map] map object with userInfo [UserInfo] holding logged in
	 *                 user details and nmasterSiteCode [int] primary key of site
	 *                 object for which the list is to be fetched
	 *                 ,nsamplestoragemappingcode: Primary key of sample storage
	 *                 mapping,ncontainerstructurecode: Indicates the primary key or
	 *                 value field used for identifying Container Structure,
	 *                 ncontainertypecode: Indicates the primary key or value field
	 *                 used for identifying Container Type.
	 * 
	 *                 Input : {"userinfo":{nmastersitecode":
	 *                 -1},"ncontainertypecode":1,"nsamplestoragemappingcode":1}
	 * 
	 * @return response entity object holding response status and list of available
	 *         Storage Structure and Sample Storage Container Path.
	 * 
	 * @throws Exception exception
	 */
	...
	public ResponseEntity<Object> getsamplemovedata(final Map<String, Object> inputMap, final UserInfo userInfo)
		...
		var strConditionInstrument = "";

		final int ninstrumentcode = (int) inputMap.getOrDefault("ninstrumentcode", 0);
		;
		...
		if (nNeedInstrumentFlow == Enumeration.TransactionStatus.YES.gettransactionstatus()) {
			...
			strConditionInstrument = " and ninstrumentcode =" + ninstrumentcode + " ";
			
			final var instumentQuery = "select i.sinstrumentid,i.ninstrumentcode from  storageinstrument si,instrument i,samplestoragemapping ssm "
					+ "where si.ninstrumentcode=i.ninstrumentcode and si.nstorageinstrumentcode=ssm.nstorageinstrumentcode and  i.ninstrumentcode>0 "
					//+ "and ssm.nprojecttypecode=" + inputMap.get("nprojecttypecode") + ""
					...
					+ " and ssm.nstatus="+ Enumeration.TransactionStatus.ACTIVE.gettransactionstatus() + "  and i.nstatus="
					+ Enumeration.TransactionStatus.ACTIVE.gettransactionstatus() + " " + "and si.nstatus="
					+ Enumeration.TransactionStatus.ACTIVE.gettransactionstatus() + " " + "and si.nregionalsitecode="
					+ userInfo.getNtranssitecode()
					+ "  GROUP BY  i.sinstrumentid, i.ninstrumentcode order by i.sinstrumentid desc ";
			...
			if (!listInstument.isEmpty()) {
				returnObject.put("lstInstrument", listInstument);
				returnObject.put("selectedInstrument", listInstument.get(listInstument.size() - 1));
			} else {
				returnObject.put("lstInstrument", null);
				returnObject.put("selectedInstrument", null);

			}
			
		}

		String strQuery = " select distinct on (ssl.nsamplestoragelocationcode) ssl.*,ts.jsondata->'stransdisplaystatus'->>'"
				+ userInfo.getSlanguagetypecode() + "'  as stransdisplaystatus from "
				+ " samplestoragelocation ssl JOIN samplestoragemapping ssm ON ssl.nsamplestoragelocationcode = ssm.nsamplestoragelocationcode "
				+ "JOIN transactionstatus ts ON ts.ntranscode = ssl.nmappingtranscode" + " where ssl.nstatus = "
				+ Enumeration.TransactionStatus.ACTIVE.gettransactionstatus() + " and ssm.nstatus = "
				+ Enumeration.TransactionStatus.ACTIVE.gettransactionstatus() + " and ssl.ncontainertypecode="
				+ inputMap.get("ncontainertypecode") + " and ssl.ncontainerstructurecode="
				+ inputMap.get("ncontainerstructurecode")
				+ " and ssl.nsamplestoragelocationcode in (select nsamplestoragelocationcode from"
				+ " samplestorageversion where nstatus=" + Enumeration.TransactionStatus.ACTIVE.gettransactionstatus()
				+ " and napprovalstatus=" + Enumeration.TransactionStatus.APPROVED.gettransactionstatus()
				+ " and nsitecode=" + userInfo.getNmastersitecode() + ")  " + strConditionInstrument + "  "
		...
		if (inputMap.containsKey("nsamplestoragelocationcode")
			...
			if (!list.isEmpty()) {
				...
				nsamplestoragelocationcode = Enumeration.TransactionStatus.NA.gettransactionstatus();
			}
		}
		...
		strQuery = " select ssm.*,ssc.nsamplestoragecontainerpathcode,ssc.jsondata->>'scontainerpath' as scontainerpath from  "
				...
				+ userInfo.getNmastersitecode() + ") " + conditionalstr
				...
				+ Enumeration.TransactionStatus.ACTIVE.gettransactionstatus() + " and nsitecode="
				+ userInfo.getNtranssitecode() + " group by nsamplestoragemappingcode) and ssm.nsitecode="
				+ userInfo.getNtranssitecode() + " " + strConditionInstrument + ";";
