...
public class SampleCollectionDAOImpl implements SampleCollectionDAO {
	...
	public ResponseEntity<Object> getSampleCollection(String fromDate, String toDate, String currentUIDate,
		...
		final String query = "select sc.scomments,sc.nsamplecollectioncode,u.sunitname,sc.sbarcodeid,sc.jsondata,COALESCE(TO_CHAR(sc.dcollectiondatetime,'"
				+ "" + userInfo.getSpgsitedatetime()
				+ "'),'') as scollectiondatetime,sc.nsampleqty,sc.scomments,sc.nprojecttypecode,sc.noffsetdcollectiondatetime from storagesamplecollection sc,unit u where u.nunitcode=sc.nunitcode and u.nstatus="
				...
				+ userInfo.getNtranssitecode() + " and " + " sc.dcollectiondatetime ::Date between '" + formattedFromDate + "' and '"
				+ formattedToDate + "' and nprojecttypecode=" + nprojecttypecode + " order by sc.nsamplecollectioncode desc";
		List<StorageSampleCollection> lstSampleCollection = jdbcTemplate.query(query,
				new StorageSampleCollection());
		
		final List<StorageSampleCollection> lstUTCConvertedDate = objMapper.convertValue(
				dateUtilityFunction.getSiteLocalTimeFromUTC(lstSampleCollection,
				Arrays.asList("scollectiondatetime"),
				Arrays.asList(userInfo.getStimezoneid()), userInfo, false, null, false),
				new TypeReference<List<StorageSampleCollection>>() {
				});
		...
		outputMap.put("SampleCollection", lstUTCConvertedDate);
	...
	public ResponseEntity<Object> createSampleCollection(Map<String, Object> inputMap, UserInfo userInfo)
		...
		if (existsSampleCollectionObj == null) {
			...
			final String dCollectionDateTime=(String) sampleCollectionObj.get("dcollectiondatetime");
			final short ntzcollectiondatetime = ((Integer) sampleCollectionObj.get("ntzcollectiondatetime")).shortValue();			
			final StorageSampleCollection objSSC = new StorageSampleCollection();				
			...
			dateList.add(Map.of(
			        "DateValue", sampleCollectionObj.get("dcollectiondatetime").toString(),
			        "TimeZoneCode", sampleCollectionObj.get("ntzcollectiondatetime")
			    ));
			
			final List<Map<String,Object>> utcConvertedDataList  = 
					dateUtilityFunction.convertToUtcBasedOnTimeZone(userInfo,dateList);
		    ...
		    final boolean flag =(boolean) utcConvertedData.get("flag"); 
 		    ...
 		    objSSC.setScollectiondatetime(formattedCollectionDateTime);			
			objSSC.setNtzcollectiondatetime(timeZoneCode);
			
			if(flag) {
			    final Short inputTimeZoneCode = (Short) utcConvertedData.get("ninputtimezonecode"); 
				objSSC.setNinputtimezonecode(inputTimeZoneCode);
				secondList.add("ninputtimezonecode");
			}else {
				secondList.add("ntzcollectiondatetime"); 
			}
			}	 
			final StorageSampleCollection convertedObject = objMapper.convertValue(
					dateUtilityFunction.convertInputDateToUTCByZone(objSSC,
							Arrays.asList("scollectiondatetime"), secondList, true, userInfo),
			...
			final String insertQuery = "insert into storagesamplecollection(nsamplecollectioncode,nprojecttypecode,nunitcode,sbarcodeid,"
					+ "jsondata,nsampleqty,dcollectiondatetime,ntzcollectiondatetime,noffsetdcollectiondatetime,"
					...
					+ convertedObject.getScollectiondatetime() + "',"
					+ objSSC.getNtzcollectiondatetime()+ "," + "'"
					+ convertedObject.getNoffsetdcollectiondatetime() + "',N'"
	...
	public ResponseEntity<Object> updateSampleCollection(Map<String, Object> inputMap, UserInfo userInfo)
		...
		if (activeSampleCollection != null) {
			...
			final String dCollectionDateTime=(String) objSampleCollection.getDcollectiondatetime().toString();
			final short ntzcollectiondatetime =  objSampleCollection.getNtzcollectiondatetime();		
			final StorageSampleCollection objSSC = new StorageSampleCollection();				
			...
			List<Map<String, Object>> dateList = new ArrayList<>();
			dateList.add(Map.of(
			        "DateValue", objSampleCollection.getDcollectiondatetime().toString(),
			        "TimeZoneCode", objSampleCollection.getNtzcollectiondatetime()
			    ));
			...
			final String updateQuery = "update storagesamplecollection set dcollectiondatetime='"
					+ convertedObject.getScollectiondatetime() + "',ntzcollectiondatetime="
					+ objSSC.getNtzcollectiondatetime() + "," + "noffsetdcollectiondatetime="
					+ convertedObject.getNoffsetdcollectiondatetime() + ",scomments=N'"
	...
	public ResponseEntity<Object> getActiveSampleCollectionById(Map<String, Object> inputMap, UserInfo userInfo)
		...
		if (activeSampleCollection != null) {
			Map<String, Object> outputMap = new HashMap<>();
			String query = " select sc.scomments,sc.nsamplecollectioncode,u.sunitname,sc.sbarcodeid,"
						 + " sc.jsondata,COALESCE(TO_CHAR(sc.dcollectiondatetime,'"+ "" + userInfo.getSpgsitedatetime()+ "'),'') as scollectiondatetime,"
						 + " sc.nunitcode,sc.nsampleqty,sc.scomments,sc.nprojecttypecode,sc.noffsetdcollectiondatetime,sc.ntzcollectiondatetime,tz.stimezoneid "
						 + " from storagesamplecollection sc "
						 + " join unit u on u.nunitcode=sc.nunitcode "
						 + " join timezone tz on tz.ntimezonecode= sc.ntzcollectiondatetime "
						 ...
						 + " and tz.nstatus=" + Enumeration.TransactionStatus.ACTIVE.gettransactionstatus() + " "
						 + " and sc.nsitecode="+ userInfo.getNtranssitecode() + " "
						 + " and sc.nsamplecollectioncode=" + nsamplecollectioncode;
					
			final List<StorageSampleCollection> lstSampleCollection = jdbcTemplate.query(query,
					new StorageSampleCollection());
			...
			List<BulkBarcodeConfigDetails> projectBarcodeConfig = jdbcTemplate.query(barcodequery,
					new BulkBarcodeConfigDetails());
			outputMap.put("jsondataBarcodeFields", projectBarcodeConfig);
			outputMap.put("activeSampleColletionByID", lstUTCConvertedDate);
			return new ResponseEntity<>(outputMap, HttpStatus.OK);
		} else {
			return new ResponseEntity<>(
					commonFunction.getMultilingualMessage(Enumeration.ReturnStatus.ALREADYDELETED.getreturnstatus(),
	...
	private String sampleCollectionAudit(final int nsamplecollectioncode, final UserInfo userinfo) {
		return "select sc.scomments,sc.nsamplecollectioncode,u.sunitname,sc.sbarcodeid,sc.jsondata,COALESCE(TO_CHAR(sc.dcollectiondatetime,'"
				+ "" + userinfo.getSpgsitedatetime()
				+ "'),'') as scollectiondatetime,sc.nsampleqty,sc.scomments,sc.nprojecttypecode,p.sprojecttypename from storagesamplecollection sc,unit u,projecttype p where u.nunitcode=sc.nunitcode and u.nstatus="
				+ Enumeration.TransactionStatus.ACTIVE.gettransactionstatus()
