		...
		//if (userInfo.getIsutcenabled()==Enumeration.TransactionStatus.YES.gettransactionstatus()) {
			...
			for (int i = 0; i < inputFieldList.size(); i++) {
				...
				//SimpleDateFormat sourceFormat1 = new SimpleDateFormat(sourceFormat);
				 if(userInfo.getNinputtimezonecode()>0) {
					 if(userInfo.getIsutcenabled()==Enumeration.TransactionStatus.YES.gettransactionstatus() 
								&& userInfo.getIstimezoneshow()==Enumeration.TransactionStatus.YES.gettransactionstatus()) {
						 map.put(timeZoneList.get(k), Short.valueOf(map.get(timeZoneList.get(k)).toString())) ;
					 }
					else if(userInfo.getIsutcenabled()==Enumeration.TransactionStatus.YES.gettransactionstatus() 
								&& userInfo.getIstimezoneshow()==Enumeration.TransactionStatus.NO.gettransactionstatus()){
						
						 map.put(timeZoneList.get(k), Short.valueOf(userInfo.getNinputtimezonecode())) ;
					}
					else if(userInfo.getIsutcenabled()==Enumeration.TransactionStatus.NO.gettransactionstatus()) {
						 map.put(timeZoneList.get(k), Short.valueOf("-1")) ;
					}
				 }else{
					 map.put(timeZoneList.get(k), Short.valueOf(userInfo.getNinputtimezonecode())) ;
				 }
...
//							objTimeZone.getStimezoneid());	
					//committed by Neeraj
					int offset = userInfo.getIsutcenabled() == Enumeration.TransactionStatus.YES.gettransactionstatus()?getCurrentDateTimeOffsetFromDate((String) map.get(inputFieldList.get(i)),
							objTimeZone.getStimezoneid()):Enumeration.TransactionStatus.NON_EMPTY.gettransactionstatus();	
					...
					map.put(timeZoneList.get(k), 
							userInfo.getIsutcenabled()==Enumeration.TransactionStatus.NO.gettransactionstatus() ? userInfo.getNinputtimezonecode() : objTimeZone.getNtimezonecode());
					...
					if (jsonObj.has("tz" + inputFieldList.get(i))) {
						...
						if(userInfo.getNinputtimezonecode()>0) {
							...
							jsonObj.put("noffset" + inputFieldList.get(i), getCurrentDateTimeOffsetFromDate(
									...
									&& userInfo.getIstimezoneshow()==Enumeration.TransactionStatus.NO.gettransactionstatus()){
								TimeZone UTZid = (TimeZone) jdbc_Template.queryForObject
										("select * from timezone where ntimezonecode ="+userInfo.getNinputtimezonecode(), new TimeZone());
								jsonObj.put("noffset" + inputFieldList.get(i), getCurrentDateTimeOffsetFromDate(
										(String) jsonObj.get(inputFieldList.get(i)),  UTZid.getStimezoneid()));
								Object value = jsonObj.get("tz" + inputFieldList.get(i));
								if (value instanceof JSONObject) {
								JSONObject innerObj = new JSONObject();
								innerObj.put("label", UTZid.getStimezoneid().toString());
								innerObj.put("value", UTZid.getNtimezonecode());
								jsonObj.put("tz"+inputFieldList.get(i), innerObj);
								}else if (value instanceof String) {
								jsonObj.put("tz"+inputFieldList.get(i), UTZid.getStimezoneid().toString());
								}
							}else if(userInfo.getIsutcenabled()==Enumeration.TransactionStatus.NO.gettransactionstatus()) {
								jsonObj.put("noffset" + inputFieldList.get(i), 0);
							}
						}else {
							jsonObj.put("noffset" + inputFieldList.get(i), 0);
						}
	...
	//Added by sonia on 18th July 2025 for jira id:ALPDJ21-18
		public  List<Map<String,Object>> convertToUtcBasedOnTimeZone( UserInfo userInfo,List<Map<String,Object>> listValue) throws Exception {			
			//final Map<String,Object>resultMap = new HashMap<String,Object>();
			final List<Map<String,Object>> ListresultMap = new ArrayList<>();
			
			final String strQuery="select ssettingvalue from settings where nsettingcode ="+Enumeration.Settings.SHOW_TIMEZONE.getNsettingcode()+" and nstatus="+Enumeration.TransactionStatus.ACTIVE.gettransactionstatus()+" ";
			final String stimeZoneNeeded = (String) jdbc_Template.queryForObject(strQuery, String.class);
			
			final int ntimeZoneNeeded=Integer.parseInt(stimeZoneNeeded);	
	        if(!listValue.isEmpty()) {
	        	
	        	for(Map<String,Object> utcConvertedData:listValue) {
	        	final Map<String,Object>resultMap = new HashMap<String,Object>();
	        	boolean inputFlag =false;
	        	final Instant instantCollectionDateTime = Instant.parse(utcConvertedData.get("DateValue").toString());	
	    	    final String formattedCollectionDateTime  =instantDateToString(instantCollectionDateTime).replace("T", " ").replace("Z", "");
	    	    resultMap.put("dateTime",formattedCollectionDateTime);
	    	    
	    		 if(userInfo.getNinputtimezonecode()>0) {
	    			if(ntimeZoneNeeded==Enumeration.TransactionStatus.NO.gettransactionstatus()) {
	    				resultMap.put("ntzdatetime",userInfo.getNinputtimezonecode());
	    			}else {
	    				resultMap.put("ntzdatetime",Short.valueOf(utcConvertedData.get("TimeZoneCode").toString()));
	    			}
	    		 }else {
	    			if(ntimeZoneNeeded==Enumeration.TransactionStatus.NO.gettransactionstatus()) {
	    				resultMap.put("ntzdatetime",userInfo.getNinputtimezonecode());						
	    				...
	    				resultMap.put("ntzdatetime",Short.valueOf(utcConvertedData.get("TimeZoneCode").toString()));
	    				resultMap.put("ninputtimezonecode",userInfo.getNinputtimezonecode());
	    				inputFlag=true;
	    			}
	    		 }
	    		resultMap.put("flag", inputFlag);
	    		resultMap.putAll(utcConvertedData);
