...
import com.agaramtech.qualis.registration.model.RegistrationTestPackage;
...
//			LOGGER.info("Date3-IG-3:"+ new Date());
		...
		// ALPDJ21-91 - 18/09/2025 - Start
		// Added by L.Subashini to insert record in intermediate table
		// along with the selected instrument of the batch
		int nbatchMasterCode = -1;
		String sbatchId = "";
		String sinstrumentId = null;
		if(Enumeration.QualisForms.BATCHCREATION.getqualisforms() == userInfo.getNformcode())
		{
			nbatchMasterCode = (int) inputMap.get("nbatchmastercode");
			sbatchId = (String) inputMap.get("sbatchid") ;
			sinstrumentId = "'" + (String) inputMap.get("sinstrumentid") + "'";
		}
		   ...
		   //JIRA: ALPD-4989-Modified by sathish for the uuid for repeat test
			...
			final	String sdmsLabSheetMaster = "insert into sdmslabsheetmaster (ntransactiontestcode,npreregno,sarno,ntransactionsamplecode,"
											     + " ntestgrouptestcode,ntestcode, nretestno,ntestrepeatcount,ninterfacetypecode,sllinterstatus,"
												 + " nusercode,nuserrolecode,suuid,nbatchmastercode,nsitecode,nstatus, sexternalintrumnetid) "
											     + " select rt.ntransactiontestcode,r.npreregno,ra.sarno, rs.ntransactionsamplecode,rt.ntestgrouptestcode,"
												 + " rt.ntestcode,rt.ntestretestno nretestno, rt.ntestrepeatno ntestrepeatcount, tm.ninterfacetypecode,"
											     + " 'A' sllinterstatus," + userInfo.getNusercode() + " nusercode," 
												 + userInfo.getNuserrole() + " nuserrolecode,"
												 + " case when "+ Enumeration.QualisForms.BATCHCREATION.getqualisforms() + "=" + userInfo.getNformcode()
												 + " then '"+  sbatchId + "' else "
												 ...
												 + " else CONCAT(r.npreregno,'-',rs.ntransactionsamplecode,'-',rt.ntestcode,'-',rt.ntestrepeatno,'-',rt.ntestretestno,'-',r.nsitecode) "
											     + " end end as suuid," + nbatchMasterCode
											     + "  nbatchmastercode,r.nsitecode,1 nstatus, " + sinstrumentId + " sexternalintrumnetid "
											     + strConcatValueSdmsLabSheetMaster;
			...
			final String sdmsLabSheetDetails = "insert into sdmslabsheetdetails (ntransactionresultcode,npreregno,sarno,"
										...
										+ " rt.ntestrepeatno,'-',rt.ntestretestno,'-',r.nsitecode) end end as suuid,"
										+  nbatchMasterCode + "  nbatchmastercode,r.nsitecode," 
...
//	ALPD-6064	Added deleteSdmsLabSheetRecords method by Vishakh (08-07-2025)
	...
	public void integrationCallService(final Map<String,Object> inputMap,final short interfacetypecode, final String preregno,final boolean needSubSample,final UserInfo userInfo) throws Exception
	{
		...
		final String dataQuery = "select jsonb_agg(a.jsondata || jsontestdata) from (SELECT max(ifs.jsonlimsmappingfield->>'default')::jsonb ||"
									...
									+ "  COALESCE(jsonb_object_agg(e_keys.e_key, f_keys.f_value) FILTER ("
									+ "  WHERE e_keys.e_key IS NOT NULL  AND f_keys.f_value IS NOT NULL),'{}' :: jsonb ) AS jsontestdata  FROM "
									+ " registration r "
									...
									+ "GROUP BY r.npreregno,rs.ntransactionsamplecode,rt.ntransactiontestcode, tfm.nintegrationfieldsettingcode,"
									+ "ifs.jsonlimsmappingfield) a ";

		//final List<Map<String,Object>> interfacerMap = jdbcTemplate.queryForList(dataQuery);
		String interfacerOrders = jdbcTemplate.queryForObject(dataQuery, String.class);
		...
		if(interfacerOrders!=null)
		{
			...
			if(interfacetypecode == Enumeration.InterFaceType.INTERFACER.getInterFaceType())
			{
				...
				interfacerRequestCall(url,interfacerOrders,preregno,userInfo);
			}
	...
	public void interfacerRequestCall(final String url,final String interfacerOrders/*final List<Map<String,Object>> interfacerMap*/,final String preregno,final UserInfo userInfo) throws Exception
	{

		ObjectMapper objectMapper = new ObjectMapper();
		//String jsonString = objectMapper.writeValueAsString(interfacerMap);
		LOGGER.info("Interfacer order record--> "+interfacerOrders);
	...
	/* HttpPost request = new HttpPost(url);
	 MultipartEntityBuilder builder = MultipartEntityBuilder.create().addTextBody("sampleorder", jsonString,ContentType.TEXT_PLAIN);
		...
		LOGGER.info("Interfacer URL --> " + url);
		httpPost.setHeader("Content-Type", "application/json");
		StringEntity entityValue = new StringEntity(interfacerOrders);
	...
	public Map<String, Object> getActiveSampleTabData(String npreregno, String ntransactionSampleCode,
		...
		case "IDS_TESTPACKAGE":
			return (Map<String, Object>) getTestPackageBySample(npreregno, userInfo).getBody();
	...
	public Map<String,Object> isValidNumeric(Map<String,Object>map, String number, String sfield, UserInfo userInfo) throws Exception {
		...
		try {
			...
			if(map.containsKey("precision") && (!map.get("precision").toString().isEmpty())) {
				...
				//ALPDJ21-133--Added by Vignesh(11-12-2025)-->Precision is not handled when importing the file
				int noAftDecimal=number.contains(".") ? number.substring(number.indexOf(".")+1).length() : 0;    
				if(noAftDecimal > precisionNo)
				{
					...
					return outputMap;
						...
						//ALPDJ21-143-->Added by Vignesh(18-12-2025)-->While importing, the precision field display was mismatched in the Sample Info icon.
						final int zeroCount=precisionNo-noAftDecimal;
						...
						if(zeroCount>0) {
							...
							outputMap.put("sprecisionvalue", sb.toString());
						}	
					}
			}
...
// ALPD-5470 Added Spl Characters (,:,) Added by Abdul
			...
			if(optionsMap.containsKey("tags") && optionsMap.get("tags")!=null) {
				...
				if(!tagsList.isEmpty()) {
					if( tagsList.stream().map(x -> x.get("text").toString()).anyMatch(x -> x.equals(dataStr)))  {
						...
						newData.put("value",dataStr);
						newData.put("label",dataStr);
						outputMap.put("objJsonData",newData);						
						outputMap.put("displaymember", dataStr);
	...
	public Map<String,Object> isComboValid(Map<String,Object> combodata,UserInfo userInfo,String field,String dataStr) throws Exception{
		...
		if (!combodata.isEmpty()) {
			...
			if (!lstData.isEmpty()) {
				...
				outmap.put("parentValueForChild",lstData.get(0));
		...
		if(isManadatoryField || !dataStr.isEmpty()) {
			...
			if(Enumeration.ReturnStatus.SUCCESS.getreturnstatus()==responseString){
				...
				if (inputType.equalsIgnoreCase("numeric") ) {
					...
					//ALPDJ21-143--Added by Vignesh(18-12-2025)-->While importing, the precision field display was mismatched in the Sample Info icon. 
					if(outmap.containsKey("sprecisionvalue")) {
					dataStr=outmap.get("sprecisionvalue").toString();
				}
				}
		...
		if (lastresetdate!=null) {
			...
			if (bYearreset) {
				...
				st = "update regsubtypeconfigversionrelease set jsondata='" + jsonobj.toString() + "'::jsonb "
						...
						+ " where nseqnoreleasenogencode=" + nseqnoreleasenogencode + ";";
				jdbcTemplate.execute(st);
			}
			else {
				if(jsonobj.has("nneedsitewisearnorelease") && jsonobj.getBoolean("nneedsitewisearnorelease")) {										
					String str = "update seqnositereleasegenerator set nsequenceno="+(list.size() + updatesequenceno)+" where nseqnoreleasenogencode=" + nseqnoreleasenogencode+" "
							+ " and nsitecode="+userinfo.getNtranssitecode()+" and nregsubtypeversionreleasecode="+nregsubtypeversionreleasecode+";";
					jdbcTemplate.execute(str);				
				}
	...
	public List<String> exportCellsValues(final List<String> strMaterial, final Map<String, Object> inputMap,
		...
		final int rows = ndirectionmastercode == Enumeration.DireactionMaster.LEFTTORIGHT.getDireactionMaster() ? nrow
				: ncolumn;
		final int columns = ndirectionmastercode == Enumeration.DireactionMaster.LEFTTORIGHT.getDireactionMaster()
				? ncolumn
				: nrow;
	...
	public ResponseEntity<Map<String,Object>> getTestPackageBySample(String spreregno, UserInfo userInfo) throws Exception {
		Map<String, Object> rtnMap = new HashMap<String, Object>();

		String strQuery = " select rtp.npreregno,rtp.nregistrationtestpackagecode, case when rarno.sarno='-' then cast(rtp.npreregno as text) else  rarno.sarno end as sarno ,"
				+ " tp.stestpackagename from registrationarno rarno,registrationtestpackage rtp,testpackage tp "
				+ " where  rarno.npreregno=rtp.npreregno and rtp.ntestpackagecode=tp.ntestpackagecode "
				+ " 	and rtp.npreregno in(" + spreregno + ") " + " and  rarno.nsitecode="
				+ userInfo.getNtranssitecode() + " and rtp.nsitecode=" + userInfo.getNtranssitecode() + ""
				...
				+ " and tp.nstatus=" + Enumeration.TransactionStatus.ACTIVE.gettransactionstatus() + ""
				+ " and rtp.ntestpackagecode>0 and tp.nsitecode=" + userInfo.getNmastersitecode() + " order by rtp.nregistrationtestpackagecode";

		final List<RegistrationTestPackage> lstRegistraTionTestPackage = jdbcTemplate.query(strQuery,
				new RegistrationTestPackage());

		rtnMap.put("RegistrationTestPackage", lstRegistraTionTestPackage);
