...
//			BeanUtils.setProperty(newObject, "versiondes", Enumeration.Sequenceintialvalue.ONE.getSequence());
	...
	public Map<String, Object> getDateFromControlProperties(final UserInfo userInfo, final String currentUIDate,
		...
		switch (dateType) {
			case "datetime":
				...
				//Added by sonia on 18th July 2025 for jira id:ALPDJ21-18 
				LocalDateTime localStartDateTime = LocalDateTime.ofInstant(startDate, ZoneOffset.UTC);
				LocalDateTime localEndDateTime = LocalDateTime.ofInstant(endDate, ZoneOffset.UTC);
	
				// Format to string without 'Z'
				String sDate = localStartDateTime.format(DateTimeFormatter.ISO_LOCAL_DATE_TIME);
				String eDate = localEndDateTime.format(DateTimeFormatter.ISO_LOCAL_DATE_TIME);

				final String fromUTCDate = dateUtilityFunction.instantDateToString(dateUtilityFunction.convertStringDateToUTC(sDate, userInfo, true));
				final String toDate = dateUtilityFunction.instantDateToString(dateUtilityFunction.convertStringDateToUTC(eDate, userInfo, true));
					
				returnObject.put("FromDate", fromUTCDate);
				returnObject.put("ToDate", toDate);		
	...
	public String getSeqfnFormat(final String mainTableName, final String secondaryTableName, final int nregtypecode,
		...
		if (objSeq.get(0).get("dseqresetdate") != null) {
	        ...
	        final LocalDateTime ldt = LocalDateTime.parse(dseqresetdate, formatter);

	        resetDate = ldt.toInstant(ZoneOffset.UTC);			
		...
		map = checkSequenceFormatNo(sequnenceno, objseq1.getNperiodcode(), secondaryTableName, resetDate, userInfo, mainTableName);
		...
		if (seqFormat != null) {
			while (seqFormat.contains("{")) {
				...
				if (subString.equals("yy") || subString.equals("YY")) {
					...
					seqFormat = seqFormat.replace('{' + subString + '}', replaceString);
				} else if (subString.equalsIgnoreCase("SiteCode")) {
					final String siteCode = jdbcTemplate.queryForObject("select ssitecode from site where nsitecode="+ userInfo.getNtranssitecode()+ " and nstatus="+ Enumeration.TransactionStatus.ACTIVE.gettransactionstatus(), String.class);
					final String replaceString = (siteCode != null && siteCode != "")? siteCode : subString;
					seqFormat = seqFormat.replace('{' + subString + '}', replaceString);
				} else {
	...
	private Map<String, Object> checkSequenceFormatNo(int nSequenceno, final int nperiodCode, final String tableName,
			final Instant dresetDate, final UserInfo userInfo, final String mainTableName) throws Exception {
		final Map<String, Object> objMap = new HashMap<String, Object>();
		final Instant dcurrentDate = dateUtilityFunction.getCurrentDateTime(userInfo);
...
//			ALPD-6044	Added nregtypecode and nregsubtypecode in filterdetail table update by Vishakh (03-07-2025)
	...
	public Map<String, Object> createFilterData(final Map<String, Object> inputMap, final UserInfo userInfo)
		...
		//L.Subashini -16/09/2025
//		final String fromUTCDate = dateUtilityFunction
//				.instantDateToString(dateUtilityFunction.convertStringDateToUTC(uiFromDate, userInfo, true));
//		final String toUTCDate = dateUtilityFunction
//				.instantDateToString(dateUtilityFunction.convertStringDateToUTC(uiToDate, userInfo, true));
...
//					.collect(Collectors.toMap(treeFormat -> treeFormat.getPrimaryKey(), treeFormat -> treeFormat));
	...
	public Map<String, Object> validateUniqueConstraint(final List<Map<String, Object>> masterUniqueValidation,
		...
		for (Map<String, Object> constraintMap : masterUniqueValidation) {
			...
			for (String key : lstKey) {
				...
				if (jsonUIData.get(key) instanceof Integer) {
					...
					data = stringUtilityFunction.replaceQuote(jsonUIData.get(key).toString());
				}
		...
		if (query.toString().length() > 1) {
			...
			//ALPDJ21-129--Added by Vignesh(08-11-2025)-->500 error occurs when the Pre-Reg has a unique combination.
			Class<?>[] finalTableName=listClass.toArray(new Class<?>[0]);
			
			                  
			List<?> lstData = getMultipleEntitiesResultSetInList(query.toString(), jdbcTemplate,finalTableName);
		...
		if (nperiodcode != Enumeration.Period.Never.getPeriod()) {
			...
			if (jsonobj.has("nneedsitewisearno") && jsonobj.getBoolean("nneedsitewisearno")) {
				str = "update seqnositearnogenerator set nsequenceno=" + (list.size() + updatesequenceno)
						...
						+ " and nsitecode=" + userInfo.getNsitecode()
	...
	private Map<String, Object> checkSequenceNo(List<String> list, int sequenceno, Instant lastresetdate,
		...
		if (lastresetdate != null) {
			...
			final LocalDateTime datetime = LocalDateTime.ofInstant(date.truncatedTo(ChronoUnit.DAYS),
					ZoneOffset.UTC);
			String currentdate = DateTimeFormatter.ofPattern("yyyy-MM-dd").format(datetime);

			final LocalDateTime resetDateTime = LocalDateTime.ofInstant(lastresetdate.truncatedTo(ChronoUnit.DAYS),
					ZoneOffset.UTC);
			...
			if (bYearreset) {
				...
				if (jsonobj.has("nneedsitewisearno") && jsonobj.getBoolean("nneedsitewisearno")) {
					st = "update seqnositearnogenerator set nsequenceno=" + (list.size())
				...
				jsonobj.put("dseqresetdate",
						dateUtilityFunction.instantDateToStringWithFormat(date, "yyyy-MM-dd"));

				//ALPDJ21-44 L.Subashini ARNo. sequence reset issue fix
...
//					dateUtilityFunction.instantDateToStringWithFormat(date, "yyyy-MM-dd HH:mm:ss"));
	...
	public Map<String, Object> getFavoriteFilterDetail(final UserInfo userInfo) throws Exception {
		...
		if (filterDetail == null) {
			...
			returnMap.putAll(Objects.requireNonNullElse(jsonData, Collections.emptyMap()));
			returnMap.putAll(Objects.requireNonNullElse(jsonTempData, Collections.emptyMap()));
...
//				List<String> lstvalue = (List<String>) CollectionUtils.collect(lst,
...
//		}
		...
		private static final String[] Ru_units = { "", "один", "два", "три", "четыре", "пять", "шесть", "семь", "восемь",
		"девять" };
		private static final String[] Ru_teens = { "десять", "одиннадцать", "двенадцать", "тринадцать", "четырнадцать", "пятнадцать",
			"шестнадцать", "семнадцать", "восемнадцать", "девятнадцать" };
		private static final String[] Ru_tens = { "", "", "двадцать", "тридцать", "сорок", "пятьдесят", "шестьдесят", "семьдесят",
			"восемьдесят", "девяносто" };
		private static final String[] Ru_thousands = { "", "Тысяча", "Миллион", "Миллиард", "Триллион" };
		
		private static final String[] Tj_units = { "", "Як", "Ду", "Се", "Чор", "Панҷ", "Шаш", "Ҳафт", "ҳашт",
				"Нӯҳ" };
		private static final String[] Tj_teens = { "Даҳ", "Ёздаҳ", "Дувоздаҳ", "Сездаҳ", "Чордаҳ", "Понздах",
				"Шонздах", "Ҳабдаҳ", "Хаждах", "Нуздаҳ" };
		private static final String[] Tj_tens = { "", "", "Бист", "сӣ", "Чил", "Панҷоҳ", "шаст", "Ҳафтод",
				"ҳаштод", "навад" };
		private static final String[] Tj_thousands = { "", "ҳазор", "миллион", "миллиард", "Триллион" };
	  ...
	  //Float and Int
	    ...
	    public static String convertToWords(double num,UserInfo userInfo) {
	        if (num == 0) {
	            return "Zero Only";
	        } else if (num < 0) {
	            return "Negative " + convertToWords(-num,userInfo);
	        }

// Round the number to the nearest integer and decimal
	        long roundedIntegerPart = Math.round(num);
	        int decimalPart = (int) Math.round((num - roundedIntegerPart) * 100);
//Convert the integer and decimal parts to words
	        String integerWords = convertToWords(roundedIntegerPart,userInfo);
	        String decimalWords = convertDecimalToWords(decimalPart);
//Build the final words
...
//----------Currnecy Mulitilingual-------------
		private static String convertToWords(long num, UserInfo userInfo) {
			if (num == 0) {
				return "";
			} else if (num < 10) {// Handle numbers less than 10
				if (userInfo.getSlanguagetypecode().equals("en-US")) {
					return units[(int) num];
				} else if (userInfo.getSlanguagetypecode().equals("ru-RU")) {
					return Ru_units[(int) num];
				} else {
					return Tj_units[(int) num];
				}
			} else if (num < 20) { // Handle numbers between 10 and 19
				if (userInfo.getSlanguagetypecode().equals("en-US")) {
					return teens[(int) (num - 10)];
				} else if (userInfo.getSlanguagetypecode().equals("ru-RU")) {
					return Ru_teens[(int) num];
				} else {
					return Tj_teens[(int) num];
				}
			} else if (num < 100) {// Handle numbers between 20 and 99
				if (userInfo.getSlanguagetypecode().equals("en-US")) {
					return tens[(int) (num / 10)] + " " + units[(int) (num % 10)];
				} else if (userInfo.getSlanguagetypecode().equals("ru-RU")) {
					return Ru_tens[(int) (num / 10)] + " " + Ru_units[(int) (num % 10)];
				} else {
					return Tj_tens[(int) (num / 10)] + " " + Tj_units[(int) (num % 10)];
				}
			} else if (num < 1000) {// Handle numbers between 100 and 999
				if (userInfo.getSlanguagetypecode().equals("en-US")) {
					return units[(int) (num / 100)] + " Hundred " + convertToWords(num % 100, userInfo);
				} else if (userInfo.getSlanguagetypecode().equals("ru-RU")) {
					return Ru_units[(int) (num / 100)] + " сто " + convertToWords(num % 100, userInfo);
				} else {
					return Tj_units[(int) (num / 100)] + " Сад " + convertToWords(num % 100, userInfo);
				}

			} else if (num < 1000000) {// Handle numbers between 1000 and 999,999
				if (userInfo.getSlanguagetypecode().equals("en-US")) {
					return convertToWords(num / 1000, userInfo) + " Thousand "
							+ convertToWords(num % 1000, userInfo);
				} else if (userInfo.getSlanguagetypecode().equals("ru-RU")) {
					return convertToWords(num / 1000, userInfo) + " Тысяча " + convertToWords(num % 1000, userInfo);
				} else {
					return convertToWords(num / 1000, userInfo) + " ҳазор " + convertToWords(num % 1000, userInfo);
				}
			} else if (num < 1000000000) {// Handle numbers between 1,000,000 and 999,999,999
				if (userInfo.getSlanguagetypecode().equals("en-US")) {
					return convertToWords(num / 1000000, userInfo) + " Million "
							+ convertToWords(num % 1000000, userInfo);
				} else if (userInfo.getSlanguagetypecode().equals("ru-RU")) {
					return convertToWords(num / 1000000, userInfo) + " Миллион "
							+ convertToWords(num % 1000000, userInfo);
				} else {
					return convertToWords(num / 1000000, userInfo) + " миллион "
							+ convertToWords(num % 1000000, userInfo);
				}
			} else { // Handle numbers larger than 999,999,999
				return "Number too large to convert";
			}
		}

	    private static String convertDecimalToWords(int num) {
	        StringBuilder decimalWords = new StringBuilder();

	        while (num > 0) {
	            int digit = num % 10;
	            decimalWords.insert(0, units[digit] + " ");
	            num /= 10;
	        }
    ...
    public static String convertToWord1(int num) {
    ...
    public List<FormFieldProperty> getFormFieldProperty(final UserInfo userInfo) throws Exception {
		final String sQuery = "select nformfieldpropertycode,  nformcode,stablename,sfieldname,nfieldsize, "
				+ " nsitecode, nstatus from formfieldproperty where nsitecode=" + userInfo.getNmastersitecode()
				+ " and  nstatus=" + Enumeration.TransactionStatus.ACTIVE.gettransactionstatus();
		final List<FormFieldProperty> genericLabelList = (List<FormFieldProperty>) jdbcTemplate.query(sQuery, new FormFieldProperty());
		return genericLabelList;
	}
