...
public class UserRoleDAOImpl implements UserRoleDAO {
	...
	public ResponseEntity<Object> createUserRole(final UserRole userRole, final UserInfo userInfo) throws Exception {
		...
		if (userRoleListByName == null) {
			...
			nsequenceNo++;
			final String insertQuery = "Insert  into userrole (nuserrolecode,suserrolename,sdescription,dmodifieddate,nsitecode,nstatus) "
			...
			;
			nseqNo++;
			final String userroleconfigquery = "insert  into userroleconfig (nuserrolecode, nneedapprovalflow, nneedresultflow, nneedtechnicianflow, nneedprojectinvestigatorflow, nneedthirdpartyflow, dmodifieddate, nsitecode, nstatus, nneedprojectflow) "
					+ "values(" + nseqNo + "," + userRoleConfig.getNneedapprovalflow() + ","
					+ userRoleConfig.getNneedresultflow() + ","+userRoleConfig.getNneedtechnicianflow()+","+userRoleConfig.getNneedprojectinvestigatorflow()+","+userRoleConfig.getNneedthirdpartyflow()+",'" + dateUtilityFunction.getCurrentDateTime(userInfo)
					+ "'," + userInfo.getNmastersitecode() + "," + " "
					+ Enumeration.TransactionStatus.ACTIVE.gettransactionstatus() + ","+userRoleConfig.getNneedprojectflow()+")";
	...
	public ResponseEntity<Object> updateUserRole(final UserRole userRole, final UserInfo userInfo) throws Exception {
		...
		final UserRole userRoleByID = getActiveUserRoleById(userRole.getNuserrolecode());
...
//							+ " and nstatus=" + Enumeration.TransactionStatus.ACTIVE.gettransactionstatus()
	...
	public ResponseEntity<Object> getUserRoleComboData(final Users objUsers, final int nuserSiteCode,
			final int nuserMultiRoleCode, final int nsiteCode, final UserInfo userInfo) throws Exception {
		String queryString = "";
		...
		return new ResponseEntity<>(jdbcTemplate.query(queryString, new UserRole()), HttpStatus.OK);
