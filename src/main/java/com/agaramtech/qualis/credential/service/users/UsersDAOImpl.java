			...
			if (activeUserById.getNlogintypecode() == Enumeration.LoginType.INTERNAL.getnlogintype()) {
				if (activeUserById.getNtransactionstatus() == Enumeration.TransactionStatus.RETIRED
					...
					final String query = " Select 'IDS_USEDASDEPUTYUSER' as Msg from usermultideputy umd, userssite us where "
							...
							+ " union all select  'IDS_EMPLOYEEATTENDENCE' as Msg from  employeeattendence e where "
							+ "e.nusercode = " + objUsers.getNusercode() + " and e.nstatus = "
							...
							+ " union all select  'IDS_AUDITPLAN' as Msg from  auditplanmembers ap where "
							+ "ap.nusercode = " + objUsers.getNusercode() + " and ap.nstatus = "
							+ Enumeration.TransactionStatus.ACTIVE.gettransactionstatus();
//							+" and nsitecode="+userInfo.getNtranssitecode(); //commented by sujatha ATE_274 01-10-2025 for delete validation in different site will not through alert issue
						...
						if (objUserFile.getSsignimgname() != null || objUserFile.getSuserimgname() != null) {
							...
							if (Enumeration.ReturnStatus.SUCCESS.getreturnstatus().equals(uploadStatus)) {
								if (userFileByUser == null) {
									...
									//ALPDJ21-132-Added by suriaprahash(21/11/25)--Resolved S3 File override issue and ensured file deletions sync with storage

										
									
									 if(userFileByUser!=null)
									 {
										final String newFileExtension =(objUserFile.getSsignimgname()!=null&& objUserFile.getSsignimgname().contains(".")) ? 
											 objUserFile.getSsignimgname().substring(objUserFile.getSsignimgname().lastIndexOf('.') + 1) : "";
										final String oldFileExtension = (userFileByUser.getSsignimgname()!= null && userFileByUser.getSsignimgname().contains("."))? 
												userFileByUser.getSsignimgname().substring(userFileByUser.getSsignimgname().lastIndexOf('.') + 1) : "";
										
										final String newFileExtensionusr =(objUserFile.getSuserimgname()!=null&& objUserFile.getSuserimgname().contains(".")) ? 
												 objUserFile.getSuserimgname().substring(objUserFile.getSuserimgname().lastIndexOf('.') + 1) : "";
										final String oldFileExtensionusr = (userFileByUser.getSuserimgname()!= null && userFileByUser.getSuserimgname().contains("."))? 
													userFileByUser.getSuserimgname().substring(userFileByUser.getSuserimgname().lastIndexOf('.') + 1) : "";
											
											
											List<String> lstFilenames = new ArrayList<>();
											if(!oldFileExtension.equals(newFileExtension)) {
												lstFilenames.add(userFileByUser.getSsignimgname());
												
											}
											if(!oldFileExtensionusr.equals(newFileExtensionusr)) {
												lstFilenames.add(userFileByUser.getSuserimgname());
											}
	...
	public ResponseEntity<Object> updateUserProfilePicture(MultipartHttpServletRequest request, UserInfo userInfo)
		...
		if (Enumeration.ReturnStatus.SUCCESS.getreturnstatus().equals(ftpResponse)) {
			...
			//start - ALPDJ21-132
			final String sQuery = "select * from userfile where nusercode = "
			...
			final UserFile objUF = (UserFile) jdbcUtilityFunction.queryForObject(sQuery, UserFile.class, jdbcTemplate);

			if(objUF != null){
 			    String suserimgftp = "";
				String s = stringUtilityFunction.replaceQuote(request.getParameter("filename"));
				final String oldFileExtension =
				        (objUF.getSuserimgftp() != null && objUF.getSuserimgftp().contains("."))
				        ? objUF.getSuserimgftp().substring(objUF.getSuserimgftp().lastIndexOf('.') + 1)
				        : "";
				String newFileExtension = s.contains(".")? s.substring(s.lastIndexOf('.') + 1) : "";
				if(!oldFileExtension.equals(newFileExtension)) {
					ftpUtilityFunction.deleteFTPFile(Arrays.asList(objUF.getSuserimgftp()), "", userInfo);
				}
				suserimgftp = s;
			}
			//end-ALPDJ21-132
		...
		if (ftpResponse.equals(Enumeration.ReturnStatus.SUCCESS.getreturnstatus())) {
			map.put("rtn", commonFunction.getMultilingualMessage(Enumeration.ReturnStatus.SUCCESS.getreturnstatus(),
					userInfo.getSlanguagefilename()));
			return new ResponseEntity<>(map, HttpStatus.OK);
		} else {
			map.put("rtn", ftpResponse);
...
//						HttpStatus.EXPECTATION_FAILED);
	...
	public ResponseEntity<?> getUpdateUserProfilePicture(UserInfo userInfo) throws Exception {
		...
		map.put("rtn", returnResponse.toString());
		return new ResponseEntity<>(map, HttpStatus.OK);
	}
