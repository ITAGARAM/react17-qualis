...
public class SiteDAOImpl implements SiteDAO {
	...
	public ResponseEntity<Object> getSiteScreen(final UserInfo userInfo) throws Exception {
		...
		final String strQuery = "select s.nsitecode,s.ntimezonecode,s.ndateformatcode,s.ssitename,s.ssiteaddress,sc.nissyncserver,sc.ninputtimezonecode,"
				...
				+ " tz.stimezoneid, tzsiteconfig.stimezoneid as sinputtimezoneid"
//				+ " case when tz.ntimezonecode=-1 then '-' else  tz.stimezoneid end,"
//				+ " case when tzsiteconfig.ntimezonecode=-1 then '-' else  tzsiteconfig.stimezoneid end as sinputtimezoneid "
				+ " ,st.ssitetypename,st.nsitetypecode  from Site s,transactionstatus ts,timezone tz,siteconfig sc,"
				+ " dateformat df,transactionstatus ts1,"
				+ " region r ,district d ,transactionstatus ts2,sitetype st,timezone tzsiteconfig  where s.nstatus = "
				+ Enumeration.TransactionStatus.ACTIVE.gettransactionstatus() + " and ts.nstatus = "
				+ Enumeration.TransactionStatus.ACTIVE.gettransactionstatus() + ""
				+ " and st.nstatus ="+Enumeration.TransactionStatus.ACTIVE.gettransactionstatus()+" and s.nsitecode>0 "
				+ " and s.ntimezonecode=tz.ntimezonecode and sc.ninputtimezonecode=tzsiteconfig.ntimezonecode and s.ndateformatcode=df.ndateformatcode "
	...
	public ResponseEntity<Object> createSiteScreen(Map<String, Object> inputMap, UserInfo userInfo) throws Exception {
		...
		//added by sujatha ATE_274 for duplicate entry on sitecode
...
//			End
			...
			//Added by sonia on 18th July 2025 for jira id:ALPDJ21-18 (ninputtimezonecode)
			insertstr = insertstr + "insert into siteconfig("
					+ "	nsitecode, ssitedatetime, ssitereportdatetime, ssitereportdate, ssitedate, spgdatetime, spgreportdatetime, nisstandaloneserver,dmodifieddate,nstatus,nissyncserver,nsitetypecode,ninputtimezonecode)"
					...
					+ Enumeration.TransactionStatus.ACTIVE.gettransactionstatus() + ","
					+ objDateFormat.getNissyncserver() + ","+objDateFormat.getNsitetypecode()+","+objDateFormat.getNinputtimezonecode()+");";
			...
			// commented by sujatha v ATE_274 on 12-09-2025 for before not getting Site Code in audit trail
...
//			}
			...
			return new ResponseEntity<>(
					commonFunction.getMultilingualMessage("IDS_SITECODEALREADYEXISTS",
			...
			//modified by sujatha ATE_274 to throw corresponding alert 
			return new ResponseEntity<>(
					commonFunction.getMultilingualMessage("IDS_SITENAMEALREADYEXISTS",
							userInfo.getSlanguagefilename()),
					HttpStatus.CONFLICT);
		}
	}
	...
	public ResponseEntity<Object> updateSiteScreen(Map<String, Object> inputMap, UserInfo userInfo) throws Exception {
		...
		if (site == null) {
			...
			if (site.getNtimezonecode() != basemasterSite.getNtimezonecode()
					|| objDateFormat.getNinputtimezonecode() != site.getNinputtimezonecode()) {

				final String query = "select 'IDS_SAMPLEREGISTRATION' as Msg from registration where nsitecode= "
						+ site.getNsitecode() + " and nstatus="
						+ Enumeration.TransactionStatus.ACTIVE.gettransactionstatus() + " union all "
						+ "select 'IDS_PROJECT' as Msg from projectmaster where nsitecode=" + site.getNsitecode()
						+ " and nstatus=" + Enumeration.TransactionStatus.ACTIVE.gettransactionstatus() + " and nprojectmastercode>0 union all "
						+ "select 'IDS_INSTRUMENT' as Msg from instrument where nregionalsitecode=" + site.getNsitecode()
						+ " and nstatus=" + Enumeration.TransactionStatus.ACTIVE.gettransactionstatus() + " union all "
						+ " select 'IDS_MATERIALINVENTORY' as Msg from materialinventory where nsitecode= "
						+ site.getNsitecode() + " and nstatus="
						+ Enumeration.TransactionStatus.ACTIVE.gettransactionstatus() + " "
						+ " union all "
						+ " select 'IDS_SAMPLESTORAGE' as Msg from samplestoragetransaction where nsitecode= "
						+ site.getNsitecode() + " and nstatus="
						+ Enumeration.TransactionStatus.ACTIVE.gettransactionstatus()
						+ " union all "
						+ " select 'IDS_PARENTSAMPLERECEIVING' as Msg from bioparentsamplereceiving where ncollectionsitecode= "
						+ site.getNsitecode() + " and nstatus="+ Enumeration.TransactionStatus.ACTIVE.gettransactionstatus();
				...
				if (objDeleteValidation.getNreturnstatus() == Enumeration.Deletevalidator.SUCCESS.getReturnvalue()) {
					...
					objDeleteValidation = projectDAOSupport.validateDeleteRecord(Integer.toString(site.getNsitecode()),
							userInfo);
					if (objDeleteValidation.getNreturnstatus() == Enumeration.Deletevalidator.SUCCESS
							.getReturnvalue()) {
						validRecord = true;
					} else {
						validRecord = false;
...
//       End			
			...
			//added by sujatha ATE_274 for duplicate entry on sitecode
				...
				//modified by sujatha ATE_274 by adding IDS to throw the corresponding alert
				return new ResponseEntity<>(
						commonFunction.getMultilingualMessage("IDS_SITENAMEALREADYEXISTS",
								userInfo.getSlanguagefilename()),
						HttpStatus.CONFLICT);
	...
	public ResponseEntity<Object> deleteSiteScreen(Site objSite, UserInfo userInfo) throws Exception {
		...
		if (site == null) {
			...
			final String query = "select 'IDS_USERSSITE' as Msg from userssite where nsitecode= " + site.getNsitecode()
					...
					+ site.getNsitecode() + " and nstatus="
					+ Enumeration.TransactionStatus.ACTIVE.gettransactionstatus()+" union all"
					+ " select 'IDS_SITEHOSPITALMAPPING' as Msg from sitehospitalmapping where nmappingsitecode= "
					+ site.getNsitecode() + " and nstatus="
					+ Enumeration.TransactionStatus.ACTIVE.gettransactionstatus()+" union all"
					+ " select 'IDS_SITEHIERARCHYNAME' as Msg from sitehierarchyconfigdetails where nnodesitecode= "
					+ site.getNsitecode() + " and nstatus="
					+ Enumeration.TransactionStatus.ACTIVE.gettransactionstatus()
					//added by sujatha ATE_274 for delete validation of site, used in manpower and siteexpirymapping
					+" union all"
					+ " select 'IDS_SITESAMPLEEXPIRYMAPPING' as Msg from siteexpirymapping where nsitemastercode= "
					+ site.getNsitecode() + " and nstatus="
					+ Enumeration.TransactionStatus.ACTIVE.gettransactionstatus()+" union all"
					+ " select 'IDS_MANPOWER' as Msg from outsourceemployee where nsitemastercode= "
					+ site.getNsitecode() + " and nstatus="
					+ Enumeration.TransactionStatus.ACTIVE.gettransactionstatus();
	...
	public Site getActiveSiteById(final int nsitecode, UserInfo userInfo) throws Exception {
		...
		final String strQuery = "select s.nsitecode,s.ntimezonecode,tz.stimezoneid ,sc.ninputtimezonecode,tzsiteconfig.stimezoneid as sinputtimezoneid," + "s.ssitename," + "s.ssitecode,"
				+ "s.ssiteaddress," + "s.scontactperson,sc.nissyncserver,"
				+ "case when s.sphoneno='null' or s.sphoneno is null then ' ' else s.sphoneno end as sphoneno," + "s.sfaxno," + "s.semail,"
				...
				+ " from Site s,transactionstatus ts,timezone tz,timezone tzsiteconfig,dateformat df ,siteconfig sc,transactionstatus ts1, region r,district d,transactionstatus ts2,sitetype st where s.nstatus = "
				+ Enumeration.TransactionStatus.ACTIVE.gettransactionstatus() + " and ts.nstatus = "
				+ Enumeration.TransactionStatus.ACTIVE.gettransactionstatus()
				...
				+ " and s.ntimezonecode=tz.ntimezonecode and sc.ninputtimezonecode=tzsiteconfig.ntimezonecode and sc.nissyncserver=ts2.ntranscode and  s.nsitecode = "
