...
public class IntegrationDAOImpl implements IntegrationDAO {
	...
	@Override
	public void interfacerRequestCall(final String url, final String interfacerOrders,final String preregno, final UserInfo userInfo) throws Exception {
		transactionDAOSupport.interfacerRequestCall(url, interfacerOrders, preregno, userInfo);
	}
	...
	public ResponseEntity<Object> updateInterfacerResult(final Map<String, Object> inputMap) throws Exception {
		...
		//ALPDJ21-123--Added by Vignesh(17-10-2025)
		//Get LimsReference Code
		int limsReferenceCode = Integer.parseInt(nestedObject.optString("TESTGRPNAME", "-1")) ;

		
		String sfieldValue = IntStream.range(0, singleFieldsArray.length())
		    .mapToObj(i -> {
		        JSONObject obj = singleFieldsArray.getJSONObject(i);
		        return "lower('" + obj.getString("sFieldName") + "')";
		...
		final String integrationQuery = "select tp.sparametersynonym,tgt.nsorter,tgp.nsorter,sm.ntestrepeatcount,sm.ntransactionsamplecode,sm.ntransactiontestcode,"
									 ...
									 + "and sd.suuid = '" + orderID + "' and lower(tp.sparametersynonym) in ("+sfieldValue+") " 
									 + "and sd.ntransactiontestcode="+limsReferenceCode+" "
		...
		Set<String> parameterCheck = new HashSet<>();
		...
		//final int minLength = Math.min(lstLimsparam.size(), arrayLength);
			...
			if (!lstLimsparam.isEmpty() && singleFieldsArray != null) {
				...
				for (int i = 0; i < lstLimsparam.size(); i++) {

					if(singleFieldsArray.length()>0)
					{

						for(int j = 0; j<arrayLength; j++ )
						{
							JSONObject field = singleFieldsArray.getJSONObject(j);
							String sresultValue = field.optString("sFieldValue", "No value").trim();
							String sparameterName = field.optString("sFieldName", "No value").trim();

							if(lstLimsparam.get(i).getSparametersynonym().equals(sparameterName))
							{
								singleFieldsArray.remove(j);
