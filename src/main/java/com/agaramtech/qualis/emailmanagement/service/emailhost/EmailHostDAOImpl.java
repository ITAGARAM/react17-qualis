...
public class EmailHostDAOImpl implements EmailHostDAO {
	...
	@Override
	public ResponseEntity<Object> getEmailHost(final UserInfo userInfo) throws Exception {
		final String strQuery = "select e.*, COALESCE(ts.jsondata->'stransdisplaystatus'->>'"
				+ userInfo.getSlanguagetypecode()
				+ "',ts.jsondata->'stransdisplaystatus'->>'en-US') as sdisplaystatus from emailhost e "
				+ " join transactionstatus ts on e.ndefaultstatus=ts.ntranscode where e.nstatus = "
				+ Enumeration.TransactionStatus.ACTIVE.gettransactionstatus() + " and e.nsitecode = "
				+ userInfo.getNmastersitecode() + " and e.nemailhostcode > 0";
	...
	public EmailHost getActiveEmailHostById(int nemailhostcode) throws Exception {
		final String strQuery = "select * from emailhost e where e.nstatus = "
				...
				+ nemailhostcode;
		final EmailHost emailHost = (EmailHost) jdbcUtilityFunction.queryForObject(strQuery, EmailHost.class,
				jdbcTemplate);
		if (emailHost != null) {
	...
	public ResponseEntity<Object> createEmailHost(EmailHost emailmanagementEmailHost, UserInfo userInfo)
		...
		if (EmailHostByName == null) {
			final String defaultQuery = "SELECT nemailhostcode FROM emailhost WHERE ndefaultstatus="
					+ Enumeration.TransactionStatus.YES.gettransactionstatus() + " and nstatus="
					+ Enumeration.TransactionStatus.ACTIVE.gettransactionstatus();
			final EmailHost emailHostByDefaultStatus = (EmailHost) jdbcUtilityFunction.queryForObject(defaultQuery,
					EmailHost.class, jdbcTemplate);
			if (emailmanagementEmailHost.getNdefaultstatus() == Enumeration.TransactionStatus.YES.gettransactionstatus() 
					&& emailHostByDefaultStatus != null) {
				final String updateQuery = "UPDATE emailhost SET ndefaultstatus="
						+ Enumeration.TransactionStatus.NO.gettransactionstatus() + " WHERE nstatus="
						+ Enumeration.TransactionStatus.ACTIVE.gettransactionstatus() + " and nemailhostcode="
						+ emailHostByDefaultStatus.getNemailhostcode();
				jdbcTemplate.execute(updateQuery);
			}
			final String sequencenoquery = "Select nsequenceno from SeqNoEmailManagement where stablename ='emailhost' and nstatus="
					+ Enumeration.TransactionStatus.ACTIVE.gettransactionstatus() + "";
			...
			final String insertquery = "Insert into emailhost (nemailhostcode, nportno, sauthenticationname, semail, shostname,"
					+ " spassword,  sprofilename, ndefaultstatus, dmodifieddate, nsitecode, nstatus) values ("
					+ nsequenceno + "," + emailmanagementEmailHost.getNportno() + ",'"
					+ emailmanagementEmailHost.getSauthenticationname() + "','" + emailmanagementEmailHost.getSemail()
					...
					+ (emailHostByDefaultStatus == null ? Enumeration.TransactionStatus.YES.gettransactionstatus() + ""
							: emailmanagementEmailHost.getNdefaultstatus())
		...
		if (emailhost == null) {
			...
			if (emailhostList.isEmpty()) {
				...
				final EmailHost emailHostByDefaultStatus = (EmailHost) jdbcUtilityFunction.queryForObject(defaultQuery,
						EmailHost.class, jdbcTemplate);
				if (emailmanagementEmailHost.getNdefaultstatus() == Enumeration.TransactionStatus.YES.gettransactionstatus()) {
					if (emailHostByDefaultStatus != null) {
						final String updateQuery = "UPDATE emailhost SET ndefaultstatus="
								+ Enumeration.TransactionStatus.NO.gettransactionstatus() + " WHERE nstatus="
								+ Enumeration.TransactionStatus.ACTIVE.gettransactionstatus() + " and nemailhostcode="
								+ emailHostByDefaultStatus.getNemailhostcode();
				...
				final String emailHostQuery = "SELECT ndefaultstatus FROM emailhost WHERE nemailhostcode="
						+ emailmanagementEmailHost.getNemailhostcode() + " and nstatus="
						+ Enumeration.TransactionStatus.ACTIVE.gettransactionstatus();
				final EmailHost objEmailHost = (EmailHost) jdbcUtilityFunction.queryForObject(emailHostQuery,
						EmailHost.class, jdbcTemplate);
				if (emailHostByDefaultStatus == null
						&& objEmailHost.getNdefaultstatus() == Enumeration.TransactionStatus.YES.gettransactionstatus()
						&& emailmanagementEmailHost.getNdefaultstatus() == Enumeration.TransactionStatus.NO.gettransactionstatus()) {
				...
				final String updateQueryString = "update emailhost set shostname=N'"
						+ stringUtilityFunction.replaceQuote(emailmanagementEmailHost.getShostname())
						+ "', ndefaultstatus=" + emailmanagementEmailHost.getNdefaultstatus() + ",sprofilename=N'"
	...
	public ResponseEntity<Object> deleteEmailHost(EmailHost emailmanagementEmailHost, UserInfo userInfo)
		...
		if (emailhost == null) {
			...
			final String deletequery = "select count(nemailhostcode) from emailconfig where nemailhostcode ="
					+ emailhost.getNemailhostcode() + " and nstatus ="
					+ Enumeration.TransactionStatus.ACTIVE.gettransactionstatus() + "";
			final int ncount = jdbcTemplate.queryForObject(deletequery, Integer.class);
			if (ncount == 0) {
				final String defaultQuery = "SELECT ndefaultstatus FROM emailhost WHERE nemailhostcode="
						+ emailhost.getNemailhostcode() + " and nstatus="
						+ Enumeration.TransactionStatus.ACTIVE.gettransactionstatus();
				final EmailHost objEmailHost = (EmailHost) jdbcUtilityFunction.queryForObject(defaultQuery,
						EmailHost.class, jdbcTemplate);
				if (objEmailHost.getNdefaultstatus() == Enumeration.TransactionStatus.YES.gettransactionstatus()) {
					return new ResponseEntity<>(commonFunction.getMultilingualMessage(
