	...
	public interface SampleSchedulingDAO {
	
		/**
		 * This interface declaration is responsible for creating a new Sample
		 * Scheduling record by accessing the DAO layer.
		 * 
		 * @param objmap A map containing:
		 * 
		 *               Sample Scheduing details (e.g., fromYear, toYear, status, etc.)
		 *               UserInfo object holding logged-in user details nmasterSiteCode
		 *               [int] representing the primary key of the site for which the
		...
		/**
		 * This interface method provides access to the DAO layer to retrieve all
		 * available samplescheduling for a specific site, filtered by date range and
		 * status.
		 *
		 ...
		 * entry in samplescheduling table.
		 * 
		...
		/**
		 * This interface declaration is used to get all the available samplescheduling
		...
		/**
		 * This interface declaration will access the DAO layer that is used to retrieve
		 * active samplescheduling object based on the specified nsampleschedulingcode.
		 * 
		 * @param nsampleschedulingcode [int] primary key of nsampleschedulingcode
		 *                              object
		...
		/**
		 * This interface declaration will access the DAO layer that is used to retrieve
		 * active sampleschedulinghistory object based on the specified
		 ...
		 * an entry in samplescheduling table.
		 * 
		 * @param inputMap holding the date fromYear,toYear,Status samplescheduling
		 *                 details and userInfo [UserInfo] holding logged in user
		 *                 details and nmasterSiteCode [int] primary key of site object
		 ...
		 * sampleschedulinglocation with respect to site
		 *
		 * @param userInfo an instance of [UserInfo] containing the logged-in user
		 *                 details and the nmasterSiteCode, which is the primary key of
		 *                 the site for which the complaints are to be fetched
		 * @param inputMap a map containing filtering criteria such as fromYear, toYear,
		 *                 and status and samplescheduling dtables records
		 *
		 * @return a ResponseEntity containing the list of sample scheduling records
		 *         associated with the specified site
		 *
		 * @throws Exception if any errors occur in the DAO layer
		 */
		public ResponseEntity<Object> getSampleSchedulingLocation(final int nprimarykey, final UserInfo userInfo)
				throws Exception;
	
		/**
		 * This interface declaration will access the DAO layer that is used to get all
		 * the available Village records with respect to City.
		 *
	...
	//modified by sujatha ATE_274 SWSM-117 by adding nsitehierarchyconfigcode for getting the approved site hierarchy config villages
		...
		/**
		 * This interface declaration will access the DAO layer that is used to delete
		 * an entry in sampleschedulinglocation table.
		 * 
		 * @param inputMap holding the sampleschedulinglocation details and userInfo
		 *                 [UserInfo] holding logged in user details and nmasterSiteCode
		 *                 [int] primary key of site object for which the list is to be
		 *                 fetched
		 * @return response entity object holding response status and data of deleted
		...
		public ResponseEntity<Object> deleteSampleSchedulingLocation(Map<String, Object> objMap, final UserInfo objUserInfo)
				throws Exception;
