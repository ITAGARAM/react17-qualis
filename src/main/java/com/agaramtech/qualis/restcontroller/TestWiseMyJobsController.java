...
public class TestWiseMyJobsController {
	...
	@PostMapping(path = "/updateTestStatus")
	public ResponseEntity<Object> updateTestStatus(@RequestBody Map<String, Object> inputMap) throws Exception {
		final ObjectMapper objMapper = new ObjectMapper();
		final UserInfo userInfo = objMapper.convertValue(inputMap.get("userinfo"), UserInfo.class);
		requestContext.setUserInfo(userInfo);
		return testwisemyjobsService.updateTestStatus(inputMap, userInfo);
	}
