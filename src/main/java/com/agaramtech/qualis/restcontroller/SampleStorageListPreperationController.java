...
public class SampleStorageListPreperationController {
	 ...
	 * @throws Exception exception
	 */
	@PostMapping(value = "/getSiteBasedInstrument")
	public ResponseEntity<Map<String, Object>> getSiteBasedInstrument(@RequestBody Map<String, Object> inputMap) throws Exception {

		final ObjectMapper objMapper = new ObjectMapper();
		final UserInfo userInfo = objMapper.convertValue(inputMap.get("userinfo"), new TypeReference<UserInfo>() {
		});
		requestContext.setUserInfo(userInfo);
		return samplestoragelistpreperationservice.getSiteBasedInstrument(inputMap, userInfo);
	}
