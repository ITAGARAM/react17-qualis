...
public class SpringSchedularDAOImpl implements SpringSchedularDAO {
	...
	public void hL7ConversionScheduler(final UserInfo userInfo) throws Exception
	{
		
		final String sURLQuery = "select ssettingvalue from settings " + " where nsettingcode = "
				+ Enumeration.Settings.HL7CONVERSION.getNsettingcode() + " and nstatus = "
				+ Enumeration.TransactionStatus.ACTIVE.gettransactionstatus();
		final String url = (String) jdbcUtilityFunction.queryForObject(sURLQuery, String.class, jdbcTemplate);
		
		
		final String sQuery="SELECT jsondata,nhl7formatconversioncode FROM hl7formatconversion "
		+" WHERE nsentstatus = "+Enumeration.TransactionStatus.DRAFT.gettransactionstatus()+" AND nsitecode = "+userInfo.getNtranssitecode()+"" 
		+" ORDER BY nhl7formatconversioncode ASC "
		+" LIMIT 1";

		Map<String, Object> result = jdbcTemplate.queryForMap(sQuery);
		
		final String jsonString = result.get("jsondata").toString();
	
		Integer nhl7formatconversioncode = (Integer) result.get("nhl7formatconversioncode");
		
		
		HttpClient httpClient = HttpClients.createDefault();
		HttpPost httpPost = new HttpPost(url);
		LOGGER.info("HL7 URL --> " + url);
		httpPost.setHeader("Content-Type", "application/json");
	/*	httpPost.setHeader("Authorization",
			    "Bearer eyJhbGciOiJIUzI1NiJ9.eyJzdWIiOiJ1c2VySW5mbyIsInNzZXNzaW9uaWQiOiJBRDYzNTMyMEQxMUE5NTRCRkVEREU3RThFMkU2OTMzOCIsImV4cGlyeSI6MSwiaWF0IjoxNzU4NjMzOTg2LCJleHAiOjE3NTg2MzQwNDZ9.SdDIO0PsCKLUI6P8sslrbpc4TqwfrCmgrmloS7EhzcY"
			);*/
		StringEntity entityValue = new StringEntity(jsonString, StandardCharsets.UTF_8);
		httpPost.setEntity(entityValue);
		HttpResponse responseValue = httpClient.execute(httpPost);
		int statusCode = responseValue.getStatusLine().getStatusCode();
		...
		if(statusCode == 200) {
			...
			final String updateIntegration = "Update hl7formatconversion set nsentstatus ="+Enumeration.TransactionStatus.SENT.gettransactionstatus()+" where nhl7formatconversioncode ="+nhl7formatconversioncode+" and nstatus = 1;";
			
			jdbcTemplate.execute(updateIntegration);			
			LOGGER.info("hl7formatconversion record sent succesfully");
		} else {
			LOGGER.info(statusCode+" Error in sending hl7formatconversion record");
		}
